...
#define DEFAULT_COLOR_THEME "ffd0222d,ffd0222d,ff140e0a,ffc9a376,fff98120,ffff7f2b,ff86983d,ff4dadb1,ffdfce89,ff94895b,ff2e2017,ffdfce89,ff412d21,ffb9ab72,ff281c14,ff2a1e16,ff281c14,ff38281d,ff191919,ff191919,ff191919,ff000000,ff141414,ffdfce89,ffdfce89,"

juce_ImplementSingleton (GLOBAL_VALUE_HOLDER)
GLOBAL_VALUE_HOLDER::GLOBAL_VALUE_HOLDER()
    : INSTANCES(0),
      LONG_MOUSE_DOWN_INTERVAL( 750 ),
      MULTIDRAG_SENSITIVITY( 0.2 ),
      SIMPLEDRAG_SENSITIVITY( 0.5 ),
      MULTIDRAG_ENABLE( true ),
      MULTIDRAG_AT_RIGHT_MOUSE
      (
#if IS_MOBILE_APP
          false
#else
      ...
      DONT_SHOW_AGAIN_2_1( false ),
      AUTO_CHECK_UPDATES(true),
      USER_ASKED_FOR_AUTO_UPDATE(false),
      QUESTION_WAS_UP(false),
      WHATS_NEW_WAS_UP(false),
...
#endif
{
    ...
    {
        COPY_OF_SESSION_FILE = SESSION_ERROR_LOG_FILE.loadFileAsString()
...
AppInstanceStore::AppInstanceStore( GstepAudioProcessor*const audio_processor_ )
    :     init_dummy( init_lock_and_feel_hack() ),
          ...
          midi_in_map( *(new MIDIInToControllerMap(this) ) ),
          pattern( *(new Pattern()) ),
          sequencer( *(new Sequencer(this)) ),
          editor_config( *(new EditorConfig(this)) ),

          launchpad_1( *(new Launchpad( this, 0 )) ),
          launchpad_2( *(new Launchpad( this, 1 )) ),
          ...
          style_step_area( new StyleStepArea( color_theme ) ),
          ...
          style_popup_editor_skip( new StylePopupEditorSkip( color_theme ) )
{
    ...
    */
    ...
    {
    #ifdef JUCE_ANDROID
        oswald_font = Font( Typeface::createSystemTypefaceFor(reinterpret_cast<const void*>( &Font("Font", 21, Font::italic) ),99));
    #else
        oswald_font = Font( Typeface::createSystemTypefaceFor(BinaryData::OswaldRegular_ttf,BinaryData::OswaldRegular_ttfSize) ).withHeight( 20 );
    #endif

    #ifdef JUCE_ANDROID
        subway_font = Font( Typeface::createSystemTypefaceFor(reinterpret_cast<const void*>( &Font("Font", 21, Font::italic) ),99));
    #else
        subway_font = Font( Typeface::createSystemTypefaceFor(BinaryData::Subway_Black_ttf,BinaryData::Subway_Black_ttfSize) ).withHeight( 20 );
    #endif
...
String AppInstanceStore::write( const XmlElement& xml_, const File& xml_doc ) const {
    ...
    if( ! xml_.writeToFile( xml_doc, "" ) )
    {
        ...
        error += "\n";
        ALERT_LOG( "Error whilst writing!",
                   error );
    }
...
String AppInstanceStore::read_error( const XmlElement*const xml_, const char*const should_version_ ) {
    String error( String("Wrong file type. Can NOT read '"
                         + xml_->getTagName()
                         + String("' as '")
...
XmlElement* get_custom_project() {
    ...
    switch( id ) {
    case 0 :
        xml = XmlDocument::parse( MemoryInputStream( BinaryData::arpeggioA_b2proj, BinaryData::arpeggioA_b2projSize, false ).readEntireStreamAsString() ).release();
        break;
    case 1 :
        xml = XmlDocument::parse( MemoryInputStream( BinaryData::arpeggioB_b2proj, BinaryData::arpeggioB_b2projSize, false ).readEntireStreamAsString() ).release();
        break;
    case 2 :
        xml = XmlDocument::parse( MemoryInputStream( BinaryData::arpeggioD_b2proj, BinaryData::arpeggioD_b2projSize, false ).readEntireStreamAsString() ).release();
        break;
    case 3 :
        xml = XmlDocument::parse( MemoryInputStream( BinaryData::_1145_b2proj, BinaryData::_1145_b2projSize, false ).readEntireStreamAsString() ).release();
        break;
    case 4 :
        xml = XmlDocument::parse( MemoryInputStream( BinaryData::_1225softSuffle_b2proj, BinaryData::_1225softSuffle_b2projSize, false ).readEntireStreamAsString() ).release();
        break;
    case 5 :
        xml = XmlDocument::parse( MemoryInputStream( BinaryData::_1564shuffle_b2proj, BinaryData::_1564shuffle_b2projSize, false ).readEntireStreamAsString() ).release();
        break;
    case 6 :
        xml = XmlDocument::parse( MemoryInputStream( BinaryData::andal_b2proj, BinaryData::andal_b2projSize, false ).readEntireStreamAsString() ).release();
        break;
    default :
        xml = XmlDocument::parse( MemoryInputStream( BinaryData::pachebel_b2proj, BinaryData::pachebel_b2projSize, false ).readEntireStreamAsString() ).release();
        break;
    }
...
String AppInstanceStore::save_standalone() {
    ...
    error += save_project( File( get_session_folder(true).getChildFile( SESSION_FILE+APPDEFF::project_file_extension ) ) );
    error += save_setup( File( get_session_folder(true).getChildFile( SESSION_FILE+APPDEFF::setup_file_extension ) ) );
...
#ifndef DEMO
    error += load_project( File( get_session_folder().getChildFile( SESSION_FILE+APPDEFF::project_file_extension ) ) );
#endif // END IF DEMO
    error += load_setup( File( get_session_folder().getChildFile( SESSION_FILE+APPDEFF::setup_file_extension ) ) );
...
String AppInstanceStore::load_plugin( const XmlElement& xml_ ) {
    ...
    // READ DAW PROJECT
    {
        ...
        XmlElement* xml_setup = xml_.getChildByName("B-Setup-2.0"); // loads old settings from project, else use file
        if( xml_setup )
...
#	ifdef B_STEP_STANDALONE
    error += save_midi_map( File( get_session_folder(true).getChildFile( SESSION_FILE+APPDEFF::mapping_file_extension) ) );
#	endif // B_STEP_STANDALONE
#endif // DEMO

    error += save_defines( File( get_session_folder(true).getChildFile( SESSION_FILE+APPDEFF::define_file_extension ) ) );
    error += save_global( File( get_session_folder(true).getChildFile( SESSION_FILE+APPDEFF::global_file_extension ) ) );
...
#	ifdef B_STEP_STANDALONE
    error += load_midi_map( File( get_session_folder().getChildFile( SESSION_FILE+APPDEFF::mapping_file_extension ) ) );
#	endif // B_STEP_STANDALONE

    error += load_defines( File( get_session_folder().getChildFile( SESSION_FILE+APPDEFF::define_file_extension ) ) );
    error += load_global( File( get_session_folder().getChildFile( SESSION_FILE+APPDEFF::global_file_extension ) ) );

    File colour_theme( get_session_folder().getChildFile( SESSION_FILE+APPDEFF::colortheme_file_extension ) );
...
String AppInstanceStore::load_project( const XmlElement& xml_ ) {
    ...
    if ( xml_.hasTagName ( APPDEFF::project_file_version ) )
    {
        ...
        if( theme != "FALSE" )
        {
            ...
            load_colour_theme( File( get_session_folder().getChildFile( SESSION_FILE+APPDEFF::colortheme_file_extension ) ) );
    ...
    else if( xml_.hasTagName( "b-step-1.2" ) )
    {
        ...
        audio_processor->speed.set_value( xml_.getIntAttribute( "Sequencer-speed", audio_processor->speed.DEFAULT ) );
        audio_processor->bpm.set_value( xml_.getIntAttribute( "Processor-bpm", audio_processor->bpm.DEFAULT ) );
        editor_config.selected_bar_id.set_value( xml_.getIntAttribute( "Editor-sel-bar", editor_config.selected_bar_id.DEFAULT ) );
    }
    else if ( xml_.hasTagName ( "b-step-1.1" ) )
    {
        String title("Deprecated Project v1.1!");
        String message("B-Step v1.1 projects are no more supported!\nTo import your old project load and save it with v1.2 and then load it with v2.x'");
        ALERT_LOG( title, message );
...
String AppInstanceStore::load_setup( const XmlElement* xml ) {
    ...
    if( xml )
    {
        ...
        if ( load_2_1 || xml->hasTagName ( APPDEFF::setup_file_version ) )
        {
            ...
            launchpad_1.channel_in = xml->getIntAttribute ( "PAD1-IN-CHANNEL", launchpad_1.channel_in.DEFAULT );
            launchpad_1.channel_out = xml->getIntAttribute ( "PAD1-OUT-CHANNEL", launchpad_1.channel_out.DEFAULT );
            launchpad_1.mode =  xml->getIntAttribute ( "PAD1-MODE", launchpad_1.mode.DEFAULT );
            launchpad_2.channel_in =  xml->getIntAttribute ( "PAD2-IN-CHANNEL", launchpad_2.channel_in.DEFAULT );
            launchpad_2.channel_out =  xml->getIntAttribute ( "PAD2-OUT-CHANNEL", launchpad_2.channel_out.DEFAULT );
        ...
        else if( xml->hasTagName( "b-step-1.1" ) )
        {
            ...
            audio_processor->is_mute.set_value( xml->getIntAttribute( "Processor-ma-mute", audio_processor->is_mute.DEFAULT ) );
            audio_processor->midi_thru.set_value( xml->getIntAttribute( "Processor-midi-thru", audio_processor->midi_thru.DEFAULT ) );
            audio_processor->sync_thru.set_value( xml->getIntAttribute( "Processor-clock-thru", audio_processor->sync_thru.DEFAULT ) );

            // Processor-midi-out-ch="1"
            // Processor-channel_in="1"
            // Processor-learn_channel_out="1"
            // Processor-learn_channel_in="1"
            audio_processor->channel_out.set_value( xml->getIntAttribute( "Processor-midi-out-ch", audio_processor->channel_out.DEFAULT ) );
            audio_processor->channel_in.set_value( xml->getIntAttribute( "Processor-channel_in-ch", audio_processor->channel_in.DEFAULT ) );
            audio_processor->learn_channel_out.set_value( xml->getIntAttribute( "Processor-learn_channel_out", audio_processor->learn_channel_out.DEFAULT ) );
            audio_processor->learn_channel_in.set_value( xml->getIntAttribute( "Processor-learn_channel_in", audio_processor->learn_channel_in.DEFAULT ) );
            ...
            launchpad_1.channel_in.set_value( xml->getIntAttribute( "Launchpad-lpd-chIN-0", launchpad_1.channel_in.DEFAULT ) );
            launchpad_1.channel_out.set_value( xml->getIntAttribute( "Launchpad-lpd-chIN-0", launchpad_1.channel_out.DEFAULT ) );
            launchpad_1.mode.set_value( xml->getIntAttribute( "Launchpad-lpd-chIN-0", launchpad_1.mode.DEFAULT ) );
            launchpad_2.channel_in.set_value( xml->getIntAttribute( "Launchpad-lpd-chIN-1", launchpad_2.channel_in.DEFAULT ) );
            launchpad_2.channel_out.set_value( xml->getIntAttribute( "Launchpad-lpd-cOUT-1", launchpad_2.channel_out.DEFAULT ) );
            launchpad_2.mode.set_value( xml->getIntAttribute( "Launchpad-lpd-mode-1", launchpad_2.mode.DEFAULT ) );
...
String AppInstanceStore::save_snapshot( const File& file_ ) const {
    return save_snapshot( file_, *bar_copy_clipboards.getUnchecked( editor_config.selected_bar_clipboard_id )->stored_bar );
}
...
String AppInstanceStore::load_snapshot( const File& file_ ) {
    ...
    return load_snapshot( file_, *bar_copy_clipboards.getUnchecked( editor_config.selected_bar_clipboard_id )->stored_bar );
}

String AppInstanceStore::load_snapshot( XmlElement& xml_ ) {
    return load_snapshot( xml_, *bar_copy_clipboards.getUnchecked( editor_config.selected_bar_clipboard_id )->stored_bar );
}
...
String AppInstanceStore::save_default_colour_theme() const {
    return save_colour_theme(File( get_session_folder(true).getChildFile( SESSION_FILE+APPDEFF::colortheme_file_extension ) ) );
}
...
public:
    ControllerPlay ( AppInstanceStore*const app_instance_store_ )
        : MONO_UIButtonController ( app_instance_store_ ),
          ...
          _drawable( Drawable::createFromImageData (BinaryData::play_svg, BinaryData::play_svgSize).release() )
    {}
};
// ************************************************************************************************
// ************************************************************************************************
...
#else
        return _app_instance_store->audio_processor->speed == APPDEF_ProcessorUserData::SPEED_NORMAL;
#endif
    }
...
public:
    ControllerPause ( AppInstanceStore*const app_instance_store_ )
        : MONO_UIButtonController ( app_instance_store_ ),
          _app_instance_store( app_instance_store_ ),
          _drawable( Drawable::createFromImageData (BinaryData::pause_svg, BinaryData::pause_svgSize).release() )
    {}
};
// ************************************************************************************************
// ************************************************************************************************
...
#else
        return _app_instance_store->audio_processor->speed == APPDEF_ProcessorUserData::SPEED_DOUBLE;
#endif
    }
...
public:
    ControllerStop ( AppInstanceStore*const app_instance_store_ )
        : MONO_UIButtonController ( app_instance_store_ ),
          _app_instance_store( app_instance_store_ ),
          _drawable( Drawable::createFromImageData (BinaryData::stop_svg, BinaryData::stop_svgSize).release() )
    {}
...
class ControllerMute : public MONO_UIButtonController
{
    ...
    void on_clicked_top() override {
        _app_instance_store->audio_processor->is_mute = !_app_instance_store->audio_processor->is_mute;
    }
...
public:
    ControllerMute ( AppInstanceStore*const app_instance_store_ )
        : MONO_UIButtonController ( app_instance_store_ ),
          _app_instance_store( app_instance_store_ ),
          _drawable( Drawable::createFromImageData (BinaryData::mute_svg, BinaryData::mute_svgSize).release() )
    {}
...
class ControllerSwingDistanceOffset : public MONO_UISliderController
{
    ...
    void get_label_text_top( String& string_ ) const override {
        ...
        APPDEF_BarStep::duration2string(_app_instance_store->pattern.swing_distance_offset,string_);
    }
...
class ControllerChordEditor : public MONO_UIButtonController
{
    ...
    const String get_help_url() override
    {
        return MANUAL_URL+"beginner/the-chord-editor";
    }
...
public:
    ControllerChordEditor ( AppInstanceStore*const app_instance_store_ )
        : MONO_UIButtonController ( app_instance_store_ ),
          _app_instance_store( app_instance_store_ ),
          _drawable( Drawable::createFromImageData (BinaryData::edit_svg, BinaryData::edit_svgSize).release() )
    {}
...
class ControllerLayer : public MONO_UIButtonController
{
    ...
    bool is_rect_style_else_fill_area() const override {
        return false;
    }
    ...
    bool using_caching() const override {
        return false;
    }
...
public:
    ControllerLayer  ( AppInstanceStore*const app_instance_store_, uint8 layer_id_, const String& name_ )
        : MONO_UIButtonController ( app_instance_store_ ),
          _layer_id( layer_id_ ),
          _name( name_ )
    {}
...
// ************************************************************************************************
// ************************************************************************************************
Controllers::Controllers( AppInstanceStore*const store_ )
    : pattern( store_ ),
      bar( store_ )
{
}
...
Controllers::ForPattern::ForPattern( AppInstanceStore*const store_ )
    : octave( new ControllerMasterOctave( store_ ) ),
      note( new ControllerMasterNote( store_ ) ),
      chord( new ControllerMasterChord( store_ ) ),

      bpm( new ControllerBPM( store_ ) ),
      mute( new ControllerMute( store_ ) ),
    ...
    bar_cc_type_2( new ControllerBarCCType( store_, 2 ) )
{
    for( unsigned int step_id = 0 ; step_id != SUM_STEPS ; ++step_id )
    {
        ...
        add_to_const_array( repeat_note_upNdown, new ControllerBarstepRepeatsNoteUpNDown( store_, step_id ) );

        add_to_const_array( repeat_is_used, new ControllerBarstepRepeatsIsUsed( store_, step_id ) );
        add_to_const_array( repeat_note_offset, new ControllerBarstepRepeatsNoteOffset( store_, step_id ) );
        add_to_const_array( repeat_velocity_offset, new ControllerBarstepRepeatsVelocityOffset( store_, step_id ) );
        add_to_const_array( repeat_duration_offset, new ControllerBarstepRepeatsDurationOffset( store_, step_id ) );
        ...
        add_to_const_array( repeats_distance_offset, new ControllerBarstepRepeatsDistanceOffset( store_, step_id ) );
        add_to_const_array( repeats_probability, new ControllerBarstepRepeatsProbability( store_, step_id ) );
        add_to_const_array( repeats_force_chord_notes, new ControllerBarstepRepeatsForce2ChordNotes( store_, step_id ) );
        ...
        add_to_const_array( pos_force_absolute_step, new ControllerBarstepPosForceToAbsolute( store_, step_id ) );
...
Controllers::ForBar::ForBarstring::ForBarstring( AppInstanceStore*const store_, uint8 barstring_id_ )
    : octave( new ControllerBarstringOctave( store_, barstring_id_ ) ),
      step( store_, barstring_id_ )
{}

Controllers::ForBar::ForBarstring::ForStep::ForStep( AppInstanceStore*const store_, uint8 barstring_id_  )
{
    for( unsigned int step_id = 0 ; step_id != SUM_STEPS ; ++step_id )
