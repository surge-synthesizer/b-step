...
uint32& AppStyle::get_background_color() {
    return default_colour;
}
uint32& AppStyle::get_foreground_color() {
    return default_colour;
}
uint32& AppStyle::get_border_color() {
    return default_colour;
}
uint32& AppStyle::get_button_border_color() {
    return button_border_color;
}
uint32& AppStyle::get_font_color() {
    return default_colour;
}
uint32& AppStyle::get_element_border_color() {
    return default_colour;
}
uint32& AppStyle::get_slider_knob_color() {
    return default_colour;
}
uint32& AppStyle::get_slider_outline_color() {
    return default_colour;
}
uint32& AppStyle::get_state_on_1_color() {
    return default_colour;
}
uint32& AppStyle::get_state_on_2_color() {
    return default_colour;
}
uint32& AppStyle::get_state_off_1_color() {
    return default_colour;
}
uint32& AppStyle::get_state_off_2_color() {
    return default_colour;
}
bool AppStyle::is_opaque() const {
    return false;
}
bool AppStyle::is_wrapper_opaque() const {
    return false;
}
const Font& AppStyle::get_font() const {
    return _font;
}
void AppStyle::set_font(Font font_) {
    _font = font_;
}
...
ColorTheme::ColorTheme()
{
    ...
    main_bg		=		Colour(0xFF483d37).darker(0.5).getARGB();

    step_area_border	=		GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR;
    step_area_bg	=		0xFF483d37;

    bar_area_border	=		Colour(GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR).darker(0.2).getARGB();
    bar_area_bg	=			Colour(step_area_bg).darker(0.5).getARGB();
...
void AppStyle::paint_outline_label(Graphics& g_,
                                   ...
                                   bool heigh_else_with_prop  )
{
    if( style_ )
    {
        ...
        g.drawFittedText (text_,
                          0,
                          owner_.proportionOfHeight (y_),
                          owner_.proportionOfWidth (w_),
                          owner_.proportionOfHeight (h_),
                          justification_, 1);
...
void ColorTheme::set_theme(String color_list_)
{
    ...
    while ( still_comma_inside )
    {
        ...
        else
        {
            ...
            *color_list.getUnchecked(counter) = Colour::fromString(color_list_.substring(0,comma_poition) ).getARGB();
...
const String ColorTheme::get_color_list() const
{
    ...
    color_list_string+= Colour(GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR).toString()+String(",");
