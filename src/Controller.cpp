...
ModelBase::ModelBase( MONO_Controller*const controller_, AppStyle*const style_ ):
    ...
    _controller( nullptr ),
    _style( nullptr ),
    _background_style( nullptr ),
    _background_color(0x00000000),
    should_repaint( false ),
    _is_manual_opaque( true ),
...
void MONO_Controller::set_model(ModelBase* model_) {
    _model = model_;
}
...
AppStyle* ModelBase::get_style() const
{
    return _style;
}
...
const ModelBase* MONO_Controller::get_model() const
{
    return _model;
}

Component* ModelBase::get_view()
{
    return _view->get_top_level_component();
}

const MONO_Controller* ModelBase::get_controller() const
{
    return _controller;
}
...
bool MONO_Controller::on_pre_mouse_down_escape(const MouseEvent&)
{
    ...
    {
        _app_instance_store->editor_config.current_editable_colour = &_model->get_style()->get_font_color();
        return true;
    }
...
void MONO_Controller::on_long_mouse_down()
{
    ...
    {
        MultidragSource*const multidrag_source = new MultidragSource( get_controller_type_ident(), this );
...
bool MONO_Controller::is_interested_in_drag_source(const DragAndDropTarget::SourceDetails& dragSourceDetails_)
{
    if( _app_instance_store->midi_in_map.is_in_learning_mode() )
        return false;
