...
public:
    AudioRecorder ( AppInstanceStore*const app_instance_store_ )
        : _app_instance_store( app_instance_store_ ),
          selected_device( -2 ),
          backgroundThread ("B-Step Audio Recorder"),
          sampleRate (0), activeWriter (nullptr),
    ...
    void startRecording (const File& file)
    {
        ...
        if (sampleRate > 0 || is_host_audio_recorder)
        {
            ...
            if (fileStream != nullptr)
            {
                ...
                //AudioFormatWriter* writer = ogg.createWriterFor (fileStream, sampleRate, 1, 16, StringPairArray(), 0);
