...
class ControllerStepLight : public MONO_UIButtonController
{
    ...
    bool are_you_simple_dragable() override {
        return true;
    }

    String _dragNdrop_ident;
    String& get_dragNdrop_ident() override {
    ...
    bool is_interested_in_drag_source ( const DragAndDropTarget::SourceDetails& dragSourceDetails_ ) override
    {
        ...
        else if ( dragSourceDetails_.description.toString().compare ( get_dragNdrop_ident() ) == 0 )
        {
            ...
            if( source_component )
            {
                BarStepCopySourceData*step_copy_source = dynamic_cast< BarStepCopySourceData* >( source_component );
                if( step_copy_source )
                {
                    ...
                    if( step_copy_source->is_multi_drag )
                    {
                        ...
                        {
                            selected_barstep.get_selected_bar().barstring(barstring_id).step(target_step_id)
                                = step_copy_source->copy_source_bar->barstring(barstring_id).step(source_step_id);
                        }
                    }
                    return true;
                }
    ...
    void item_dropped_top ( const DragAndDropTarget::SourceDetails& dragSourceDetails_ ) override
    {
        ...
        {
            BarStepCopySourceData*const source = reinterpret_cast< BarStepCopySourceData*const > ( dragSourceDetails_.sourceComponent.get() );
            if( source )
                CoreCopy::barstep( *source->copy_source_barstep, selected_barstep.get(), *source->copy_source_bar,selected_barstep.get_selected_bar() );
        }
    }
    ...
    String& get_multi_dragNdrop_ident() override {
        return get_dragNdrop_ident();
    }

    // RUBBER
    bool are_you_rubberable() override
    {
        return true;
    }
    ...
    void on_clicked_top() override {
        selected_barstep.get().mute.invert();
    }
...
public:
    ControllerStepLight ( AppInstanceStore* const app_instance_store_, uint8 step_id_ )
        : MONO_UIButtonController ( app_instance_store_ ),
          ...
          is_multidrag_source( false ),
          _dragNdrop_ident( BARSTEP_DRAG_N_DROP_IDENT )
    {}
};
...
class ControllerBarSelect : public MONO_UIButtonController
{
    ...
    void on_param_via_changed() const override {
	bar.set_selected_bar();
    }
    ...
    bool is_interested_in_drag_source ( const DragAndDropTarget::SourceDetails& dragSourceDetails_ ) override
    {
        ...
        else if ( dragSourceDetails_.description.toString().compare ( BAR_CLIPBOARD_DRAG_N_DROP_IDENT ) == 0 )
        {
            Component*source = dragSourceDetails_.sourceComponent.get();
            if( source )
            {
...
class ControllerBarRepeat : public MONO_UISliderController
{
    ...
    PodParameterBase* get_parameter() const override {
        return &bar.get().repeats;
    }

    void get_label_text_top( String& string_ ) const override {
        string_ = String(bar.get().repeats);
    }
...
public:
    ControllerBarCCType ( AppInstanceStore*const app_instance_store_, uint8 cc_set_id_ )
        : MONO_UISliderController ( app_instance_store_ ),
          selected_bar( app_instance_store_ ),
...
class ControllerBarSongReset : public MONO_UIButtonController
{
    ...
    virtual PodParameterBase* get_parameter() const override {
        return &bar.get().pos_reset;
    }
...
class ControllerBarPlayRandom : public MONO_UIButtonController
{
    ...
    PodParameterBase* get_parameter() const override {
        return &bar.get().random_playback;
    }
...
class ControllerBarStepShift : public MONO_UISliderController
{
    ...
    void on_value_changed( int v_ ) override
    {
        ...
        // copy all
        for( int8 source_step_id = count_from ; source_step_id != count_to ; source_step_id+=count_up_or_down )
        {
