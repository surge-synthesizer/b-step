...
ProcessorUserData::ProcessorUserData()
    ...
    (
        ...
        &latency_corretion_clocks,
        &latency_corretion_ms,
...
Pattern::Pattern ()
    ...
    (
        ...
        &swing_position,
        &swing_distance_offset,
        &swing_velocity_offset,
...
BarStep::BarStep ( uint8 step_id_ )
    : id ( step_id_ ),
      ...
      repeats( id ),
      repeat_distance( id ),
      repeat_velocity_offset( id ),
      repeat_duration_offset( id ),
      repeat_note_offset( id ),
      skip_repeat( id ),
      repeat_note_upNdown( id ),
      dont_roll_repeat( id ),
      repeat_distance_offset( id ),
      repeat_probability( id ),
      repeat_force_chord_notes( id ),

      pos_entry_point( id ),
      pos_reset_point( id ),
      pos_force_to_absolute_step( id ),
      ...
      (
          ...
          &repeats,
          &repeat_distance,
          &repeat_velocity_offset,
          &repeat_duration_offset,
          &repeat_note_offset,
          &skip_repeat,
          &repeat_note_upNdown,
          &dont_roll_repeat,
          &repeat_distance_offset,
          &repeat_probability,
          &repeat_force_chord_notes,

          &pos_entry_point,
          &pos_reset_point,
          &pos_force_to_absolute_step
      )
{
    // CC VALS
...
Step::Step ( uint8 sid, uint8 id_ )
    : id ( id_ ),
      ...
      is_mute
      (
          String((sid == 0)?"-G-" :(sid==1)?"-D-" :(sid==2)?"-A-" :"-E-")+=(id+1)
      ),
...
UIBarClipboardSettings::UIBarClipboardSettings( uint8 clipboard_id_ )
    : id( clipboard_id_ ),
      ...
      is_copy_steps( clipboard_id_ ),
      is_copy_string_octave( clipboard_id_ ),
      is_copy_step_duration( clipboard_id_ ),
      is_copy_step_velocity( clipboard_id_ ),
      is_copy_bar_solo( clipboard_id_ ),
      is_copy_bar_groups( clipboard_id_ ),
      is_copy_layer_1( clipboard_id_ ),
      is_copy_layer_2( clipboard_id_ ),
      is_copy_layer_3( clipboard_id_ ),
      is_copy_layer_4( clipboard_id_ ),
      is_copy_layer_5( clipboard_id_ ),
      is_copy_layer_6( clipboard_id_ ),
      ...
      (
          &is_copy_steps,
          &is_copy_string_octave,
          &is_copy_step_duration,
          &is_copy_step_velocity,
          &is_copy_bar_solo,
          &is_copy_bar_groups,
          &is_copy_layer_1,
          &is_copy_layer_2,
...
template<class child_container_t>
void import_childs_by_default( const child_container_t& childs_ )
{
    for( int i = 0 ; i != childs_.size() ; ++i )
    {
...
template<class T>
void import_my_params_from( const T& owner_, XmlElement& xml_ )
{
...
template<class T>
void import_my_params_by_default( const T& owner_ )
{
...
void export_childs_to( const OwnedArray< child_t >& childs_, XmlElement& xml_ )
{
    for( int i = 0 ; i != childs_.size() ; ++i )
    {
...
template<class T>
void export_my_params_to( const T& owner_, XmlElement& xml_ )
{
    Array< typename T::appdeff_t::IDS > param_list = T::appdeff_t::get_project_parameter_list();
    for( int i = 0 ; i != param_list.size() ; ++i )
    {
...
void ProcessorUserData::import_from( const XmlElement& xml_ )
{
    MY_PARAMS_IMPORTER_AND_INITER
}
void ProcessorUserData::import_by_default()
{
...
void MIDIUserData::import_from( const XmlElement& xml_ )
{
    MY_PARAMS_IMPORTER_AND_INITER
}
void MIDIUserData::import_by_default()
{
    import_my_params_by_default(*this);
}
...
void ChordSet::import_by_default(bool)
{
    import_childs_by_default( _chords );
}
...
void Chord::import_from(const XmlElement& xml_)
{
    MY_PARAMS_IMPORTER_AND_INITER_BY_ID
}
void Chord::import_by_default()
{
    import_my_params_by_default(*this);
}
...
void BarCCSet::import_from(const XmlElement& xml_)
{
    MY_PARAMS_IMPORTER_AND_INITER_BY_ID
}
void BarCCSet::import_by_default()
{
    import_my_params_by_default(*this);
}
...
void BarStepCCVals::import_from(const XmlElement& xml_)
{
    MY_PARAMS_IMPORTER_AND_INITER_BY_ID
}
void BarStepCCVals::import_by_default()
{
    import_my_params_by_default(*this);
}
...
void Step::import_from(const XmlElement& xml_)
{
    MY_PARAMS_IMPORTER_AND_INITER_BY_ID
}
void Step::import_by_default()
{
    import_my_params_by_default(*this);
}
...
void UIUserData::import_from(const XmlElement& xml_)
{
    MY_PARAMS_IMPORTER_AND_INITER
}
void UIUserData::import_by_default()
{
    import_my_params_by_default(*this);
}
...
void UIBarClipboardSettings::import_by_default()
{
    import_my_params_by_default(*this);
}
...
void Chord::import_from_1_2(const XmlElement& xml )
{
    ...
    offset_g.set_value( xml.getIntAttribute( my_prefix+"Chord-chd-strG-" + String(id), offset_g.DEFAULT ) );
    offset_d.set_value( xml.getIntAttribute( my_prefix+"Chord-chd-strD-" + String(id), offset_d.DEFAULT ) );
    offset_a.set_value( xml.getIntAttribute( my_prefix+"Chord-chd-strA-" + String(id), offset_a.DEFAULT ) );
    offset_e.set_value( xml.getIntAttribute( my_prefix+"Chord-chd-strE-" + String(id), offset_e.DEFAULT ) );
    offset_all.set_value( xml.getIntAttribute( my_prefix+"Chord-chd-all-" + String(id), offset_all.DEFAULT ) );
}
...
void Bar::import_from_1_2(const XmlElement& xml, String& parent_prefix_ )
{
    ...
    octave_offset.set_value( xml.getIntAttribute( "Bar-bar-octv-" + String(id), octave_offset.DEFAULT ) );
...
void BarStep::import_from_1_2(const XmlElement& xml, String& parent_prefix_)
{
    ...
    velocity.set_value( xml.getIntAttribute( parent_prefix_+String("BarStep")+"-stp-velo-"+String(id), velocity.DEFAULT ) );

    int old_duration = xml.getIntAttribute( parent_prefix_+String("BarStep")+"-stp-dura-"+String(id), duration.DEFAULT );
    switch( old_duration )
    {
    case 1 :
        duration = 6;
...
void Barstring::import_from_1_2(const XmlElement& xml, String& parent_prefix_)
{
    ...
    // my importer
    octave_offset.set_value( xml.getIntAttribute( my_prefix+"-str-octv-"+String(id), octave_offset.DEFAULT ) );
...
void Step::import_from_1_2(const XmlElement& xml, String& parent_prefix_)
{
    ...
    is_mute.set_value( xml.getIntAttribute( parent_prefix_+"Step-stp-mute-"+String(id), is_mute.DEFAULT ) );
}
