...
static inline uint8 get_mode_color()
{
    return LAUNCHPAD_AMBER_FULL;
}

static inline uint8 get_pressed_option_color()
{
    return LAUNCHPAD_AMBER_FULL;
}

static inline uint8 get_mode_arrow_color()
{
    return LAUNCHPAD_AMBER_LOW;
}

static inline uint8 get_master_octave_up_color ( const Pattern& )
{
    return LAUNCHPAD_AMBER_MID;
}
static inline uint8 get_master_octave_down_color ( const Pattern& )
{
    return LAUNCHPAD_AMBER_LOW;
}

static inline uint8 get_master_note_up_color ( const Pattern& )
{
    return LAUNCHPAD_RED_MID;
}
static inline uint8 get_master_note_down_color ( const Pattern& )
{
    return LAUNCHPAD_RED_LOW;
}
static inline uint8 get_master_chord_type_up_color ( const Pattern& )
{
    return LAUNCHPAD_GREEN_MID;
}
...
Launchpad::Button::Button ( uint8 hard_button_note_number_, int8 )
    :  cache_launchpad_controller_value ( -1 ), // UNKNOWN
       message ( MidiMessage::noteOn ( 1, hard_button_note_number_, uint8 ( 0 ) ) ),
       is_pressed ( false )
{}
...
void Launchpad::process_colormessage_for_button (
    uint8 color_,
    uint8 channel_,
...
void Launchpad::process_colormessage_CC_for_button (
    uint8 color_,
    uint8 channel_,
    Launchpad::Button& button_,
    Array< MidiMessage* >& messages_,
    bool force_ )
{
    if ( color_ != button_.cache_launchpad_controller_value )
    {
        button_.message = MidiMessage::controllerEvent ( channel_out, button_.message.getControllerNumber(), color_ );
        button_.cache_launchpad_controller_value = color_;
        messages_.add ( &button_.message );
    }
}
...
{
    const Bar& selected_bar = _app_instance_store->pattern.bar(_app_instance_store->editor_config.selected_bar_id);
    ...
    for ( uint8 hard_row_id = 0; hard_row_id != SUM_HARD_BUTTON_ROWS ; ++hard_row_id ) {
        // OPTION BUTTONS TOP
        {
            ...
            case FIXED_LAUNCHPAD_TOP_UP : // speed
                process_colormessage_CC_for_button ( get_speed_up_color ( *_app_instance_store->audio_processor ), channel_out, option_button_top, messages_ );
                break;
            case FIXED_LAUNCHPAD_TOP_DOWN :
                process_colormessage_CC_for_button ( get_speed_down_color ( *_app_instance_store->audio_processor ), channel_out, option_button_top, messages_ );
                break;
            case FIXED_LAUNCHPAD_TOP_LEFT :
                process_colormessage_CC_for_button ( get_mode_arrow_color(), channel_out, option_button_top, messages_ );
                break;
            ...
            case FIXED_LAUNCHPAD_TOP_SESSION: // SESSION
                if ( mode == LAUNCHPAD_MODE_SESSION )
                    process_colormessage_CC_for_button ( get_mode_color(), channel_out, option_button_top, messages_ );
                else
                    process_colormessage_CC_for_button ( LAUNCHPAD_OFF, channel_out, option_button_top, messages_ );
                break;
            ...
            case FIXED_LAUNCHPAD_TOP_MIXER: // SHOW PAGE 1 or 2 1
                process_colormessage_CC_for_button ( get_scroll_color ( _current_scroll_offset ), channel_out, option_button_top, messages_ );
                break;
            }
        }
        ...
        // OPTION BUTTONS ( RIGHT )
        {
            ...
            if ( _buttons_right.getReference( hard_row_id ).is_pressed )
                ;//process_colormessage_for_button( get_pressed_option_color(), _channel_out, option_button, messages );
            else
                ;//process_colormessage_for_button( LAUNCHPAD_OFF, _channel_out, option_button, messages );
            ...
            if ( _current_scroll_offset == SCROLL_OFFSET_PAGE_1 ) {
                ...
                case FIXED_LAUNCHPAD_PAGE_1_SHIFT_CARLA_CC_FIX :
                    if ( _buttons_right.getReference( hard_row_id ).is_pressed )
                        process_colormessage_for_button ( get_pressed_option_color(), channel_out, option_button, messages_ );
                    else
                        process_colormessage_for_button ( LAUNCHPAD_OFF, channel_out, option_button, messages_ );
                    break;
                case FIXED_LAUNCHPAD_PAGE_1_MASTER_OCTAVE_UP :
                    process_colormessage_for_button ( get_master_octave_up_color ( pattern ), channel_out, option_button, messages_ );
                    break;
                case FIXED_LAUNCHPAD_PAGE_1_MASTER_OCTAVE_DOWN :
                    process_colormessage_for_button ( get_master_octave_down_color ( pattern ), channel_out, option_button, messages_ );
                    break;
                case FIXED_LAUNCHPAD_PAGE_1_MASTER_NOTE_UP :
                    process_colormessage_for_button ( get_master_note_up_color ( pattern ), channel_out, option_button, messages_ );
                    break;
                case FIXED_LAUNCHPAD_PAGE_1_MASTER_NOTE_DOWN :
                    process_colormessage_for_button ( get_master_note_down_color ( pattern ), channel_out, option_button, messages_ );
                    break;
                case FIXED_LAUNCHPAD_PAGE_1_MASTER_CHORD_UP :
                    process_colormessage_for_button ( get_master_chord_type_up_color ( pattern ), channel_out, option_button, messages_ );
                    break;
                case FIXED_LAUNCHPAD_PAGE_1_MASTER_CHORD_DOWN :
                    process_colormessage_for_button ( get_master_chord_type_down_color ( pattern ), channel_out, option_button, messages_ );
                    break;
        ...
        // MATRIX BUTTONS
        {
            for ( uint8 hard_step_id = 0; hard_step_id != SUM_HARD_BUTTON_STEPS ; ++hard_step_id ) {
                ...
                const uint8 session_mode_bar_id = ( mode == LAUNCHPAD_MODE_SESSION ) ? hard_step_id : releative_step_id_;

                if ( hard_row_id == FIXED_LAUNCHPAD_ROW_STEP_RUNLIGHT ) {
                    // RUN LIGHT STEP
                    // TODO move to function start
                    uint8 running_step_id = _app_instance_store->sequencer.get_running_step_id(0); // TODO, for groups?
                    if ( releative_step_id_ == running_step_id )
                        process_colormessage_for_button ( get_launchpad_running_step_color ( true ), channel_out, button, messages_ );
                    else if ( mode == LAUNCHPAD_MODE_SESSION
                              && releative_step_id_ == running_step_id+1 ) {
                        process_colormessage_for_button ( get_launchpad_running_step_color ( true ), channel_out, button, messages_ );
                    } else
                        process_colormessage_for_button ( get_launchpad_running_step_color ( false ), channel_out, button, messages_ );
                }
                /*
                // TODO removed standard solo bar things
                              // BAR SELECT & RUN LIGHT
                                  ...
                                  // SOLO BAR
                                  if ( session_mode_bar_id == _app_instance_store->sequencer.get_solo_bar_id() )
                                      process_colormessage_for_button ( get_launchpad_solo_bar_color ( true ), channel_out, button, messages );
                                  // RUN LIGHT BAR
                                  else if ( session_mode_bar_id == _app_instance_store->sequencer.get_running_bar_id() )
                                      process_colormessage_for_button ( get_launchpad_running_bar_color ( true ), channel_out, button, messages );
                                  // SELECTED BAR LIGHT
                                  else if ( selected_bar.id == session_mode_bar_id )
                                      process_colormessage_for_button ( get_launchpad_selected_bar_color ( true ), channel_out, button, messages );
                                  else
                                      process_colormessage_for_button ( get_launchpad_selected_bar_color ( false ), channel_out, button, messages );
                              }
                              */
                // STEP BUTTONS ( every second step on session )
                    ...
                    uint8 current_step_color = get_launchpad_steps_color ( selected_bar.barstring ( barstring_id ).step ( releative_step_id_ ).is_mute );
                    process_colormessage_for_button ( current_step_color, channel_out, button, messages_ );
                }
                // BAR OCTAVE UP
                    ...
                    process_colormessage_for_button ( current_chord_up_color, channel_out, button, messages_ );
                }
                // BAR OCTAVE DOWN
                    ...
                    process_colormessage_for_button ( current_chord_down_color, channel_out, button, messages_ );
                }
                // OCTAVE UP
                ...
                // DURATION
                    ...
                    uint8 current_duration_color = get_launchpad_duration_color ( selected_bar.barstep ( releative_step_id_ ).duration );
                    process_colormessage_for_button ( current_duration_color, channel_out, button, messages_ );
                }
                // VELOCITY
                else if ( relative_row == RELEATIVE_LAUNCHPAD_ROW_VECLOCITY ) {
                    uint8 current_velocity_color = get_launchpad_velocity_color ( selected_bar.barstep ( releative_step_id_ ).velocity );
                    process_colormessage_for_button ( current_velocity_color, channel_out, button, messages_ );
                }
                // REPATS
                else if ( relative_row == RELEATIVE_LAUNCHPAD_ROW_REPATS ) {
                    uint8 current_repeats_color = get_launchpad_bar_repeat_color ( _app_instance_store->pattern.bar ( session_mode_bar_id ) );
                    process_colormessage_for_button ( current_repeats_color, channel_out, button, messages_ );
                }
            }
        }
    }
...
static inline uint8 get_pref_valid_autochain_bar_id ( const Pattern& pattern_, unsigned int from_bar_id_ )
{
...
static inline uint8 get_next_valid_autochain_bar_id ( const Pattern& pattern_, unsigned int from_bar_id_ )
{
    // check all bars from current + 1
...
void Launchpad::process ( const MidiMessage& message_ )
{
    ...
    if ( message_.isController() ) {
        if ( message_.getControllerValue() != LAUNCHPAD_OFF ) {
            ...
            case FIXED_LAUNCHPAD_TOP_UP :
                if ( _app_instance_store->audio_processor->speed == APPDEF_ProcessorUserData::SPEED_HALF )
                    _app_instance_store->audio_processor->speed = APPDEF_ProcessorUserData::SPEED_NORMAL ;
                else if ( _app_instance_store->audio_processor->speed == APPDEF_ProcessorUserData::SPEED_NORMAL )
                    _app_instance_store->audio_processor->speed = APPDEF_ProcessorUserData::SPEED_DOUBLE;
                else if ( _app_instance_store->audio_processor->speed == APPDEF_ProcessorUserData::SPEED_DOUBLE )
            ...
            case FIXED_LAUNCHPAD_TOP_LEFT:
                _app_instance_store->editor_config.selected_bar_id
                    = get_pref_valid_autochain_bar_id ( _app_instance_store->pattern, _app_instance_store->editor_config.selected_bar_id );
                break;
            case FIXED_LAUNCHPAD_TOP_RIGHT_CARLA_FIX :
                ;
            case FIXED_LAUNCHPAD_TOP_RIGHT:
                _app_instance_store->editor_config.selected_bar_id
                    = get_next_valid_autochain_bar_id ( _app_instance_store->pattern, _app_instance_store->editor_config.selected_bar_id );
                break;
    ...
    // standard button handling
        ...
        const uint8 hard_button_step = controller_number % 16; // button 1:1 has value 0 --- button 3:1 has 33
        ...
        // on session we handle the bar from 1 to 8 only on user_2 we take 9 to 16
        const uint8 session_mode_bar_id = ( mode == LAUNCHPAD_MODE_SESSION ) ? hard_button_step : relative_button_id;
        ...
        if ( message_.isNoteOn() ) {
            if ( hard_button_step == LAUNCHPAD_RIGHT_OPTION_COLUM ) {
            ...
            } else if ( hard_button_row == FIXED_LAUNCHPAD_ROW_STEP_RUNLIGHT ) { // runlight
                if ( _buttons_right.getReference( FIXED_LAUNCHPAD_ROW_STEP_RUNLIGHT ).is_pressed ) {
                    switch ( hard_button_step ) {
                    case 0:
                        if ( _app_instance_store->audio_processor->speed == GstepAudioProcessor::ProcessorUserData::appdeff_t::SPEED_HALF )
                            _app_instance_store->audio_processor->speed = GstepAudioProcessor::ProcessorUserData::appdeff_t::SPEED_NORMAL;
                        else if ( _app_instance_store->audio_processor->speed == GstepAudioProcessor::ProcessorUserData::appdeff_t::SPEED_NORMAL )
                            _app_instance_store->audio_processor->speed = GstepAudioProcessor::ProcessorUserData::appdeff_t::SPEED_DOUBLE;
                        else if ( _app_instance_store->audio_processor->speed == GstepAudioProcessor::ProcessorUserData::appdeff_t::SPEED_DOUBLE )
                            _app_instance_store->audio_processor->speed = GstepAudioProcessor::ProcessorUserData::appdeff_t::SPEED_HALF;
                        break;
                    case 1:
                        if ( _app_instance_store->audio_processor->speed == GstepAudioProcessor::ProcessorUserData::appdeff_t::SPEED_HALF )
                            _app_instance_store->audio_processor->speed = GstepAudioProcessor::ProcessorUserData::appdeff_t::SPEED_DOUBLE;
            ...
            } else if ( hard_button_row == FIXED_LAUNCHPAD_ROW_STEP_BAR_SELECT ) { // bar select
                ...
                else if ( hard_button_step != LAUNCHPAD_RIGHT_OPTION_COLUM )
                {
                    ...
                    for( int id = 0; id != _buttons.getReference(FIXED_LAUNCHPAD_ROW_STEP_BAR_SELECT).size(); ++id )
                    {
                        if( _buttons.getReference(FIXED_LAUNCHPAD_ROW_STEP_BAR_SELECT).getReference(id).is_pressed )
                        {
                            bar_button_pressed = id;
                        }
                    }
                    if( bar_button_pressed != -1 )
                        _app_instance_store->pattern.bar( session_mode_bar_id ) = _app_instance_store->pattern.bar( bar_button_pressed );
                    else
                        _buttons.getReference( FIXED_LAUNCHPAD_ROW_STEP_BAR_SELECT).getReference(session_mode_bar_id).is_pressed = true;
                ...
                if ( relative_row <= RELEATIVE_LAUNCHPAD_END_ROW_STEPS ) {
                    Bar& selected_bar = _app_instance_store->pattern.bar(_app_instance_store->editor_config.selected_bar_id);
        ...
        // will be ignored for changes in the core
            ...
            if ( hard_button_step == LAUNCHPAD_RIGHT_OPTION_COLUM ) { // bar select
                _buttons_right.getReference( hard_button_row ).is_pressed = false;
            }
            else if ( hard_button_row == FIXED_LAUNCHPAD_ROW_STEP_BAR_SELECT ) { // bar select
                if ( hard_button_step != LAUNCHPAD_RIGHT_OPTION_COLUM )
                {
                    _buttons.getReference(FIXED_LAUNCHPAD_ROW_STEP_BAR_SELECT).getReference(session_mode_bar_id).is_pressed = false;
                }
            }
        }
    }
