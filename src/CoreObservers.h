...
public:
    inline Pattern& get_selected_pattern() const
    {
        return _app_instance_store->pattern;
    }

    inline Pattern& get() const
    {
        return _app_instance_store->pattern;
    }

    SelectedPatternObserver( AppInstanceStore*const app_instance_store_ ) : _app_instance_store( app_instance_store_ ) {}
...
public:
    ...
    SelectedBarObserver( AppInstanceStore*const app_instance_store_ )
        : SelectedPatternObserver( app_instance_store_ ),
...
public:
    ...
    Bar& get_bar() const
    {
        return _app_instance_store->pattern.bar( _bar_id );
    }
    ...
    {
        return _app_instance_store->sequencer.get_probabliy_next_bar( get().group )
               == _bar_id;
    }

    bool is_selected_bar() const
    {
        return get_selected_bar().id == _bar_id;
    }

    void set_selected_bar() const
    {
        _app_instance_store->editor_config.selected_bar_id = _bar_id;
    }

    BarObserver( AppInstanceStore*const app_instance_store_, uint8 bar_id_ )
        : SelectedBarObserver( app_instance_store_ ),
          _app_instance_store( app_instance_store_ ),
          _bar_id( bar_id_ )
    {}
};
...
public:
    const uint8 step_id;
    inline BarStep& get_selected_barstep() const
    {
        return get_selected_bar().barstep(step_id);
    }

    inline BarStep& get() const
    {
        return get_selected_bar().barstep(step_id);
    }
    ...
    {
        return _app_instance_store->sequencer.get_running_step_repeat_as_count_down( SelectedBarObserver::get().group, step_id);
    }

    bool is_repeat_at_step_already_processed() const
    {
        return _app_instance_store->sequencer.is_step_repeat_already_processed( SelectedBarObserver::get().group, step_id );
    }

    Bar& get_running_bar() const
    {
        return _app_instance_store->pattern.bar( _app_instance_store->sequencer.get_running_bar_id( SelectedBarObserver::get().group ) );
    }

    uint8 is_running_step_id() const
    {
        return _app_instance_store->sequencer.get_running_step_id( SelectedBarObserver::get().group ) == step_id;
    }
...
protected:
    inline Step& get_selected_step() const
    {
        return get_selected_barstring().step(step_id);
    }

public:
    uint8 step_id;

    inline Step& get() const
    {
        return get_selected_barstring().step(step_id);
    }
    ...
    SelectedStepObserver( AppInstanceStore*const app_instance_store_, uint8 barstring_id_, uint8 step_id_ ) :
