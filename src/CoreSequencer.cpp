...
public:
    ...
    {
        /** NOTE if you like to have it i realtime, just replace the init's with the data from the source barstep */
        ...
        if( _has_processable_data )
        {
            if( is_processing_valid() )
            {
                if( _current_interval < 1 )
                {
                    // INTERVAL
                    {
                        int16 new_interval = APPDEF_BarStep::trans_duration2clocks( _init_interval )
                                             + (APPDEF_BarStep::trans_duration2clocks( _init_interval_offset )*_repeat_counter);
                    ...
                    // DURATION, VELO
                    {
                        ...
                        _current_duration_offset += APPDEF_BarStep::trans_duration2clocks( _init_duration_offset );
                    }
...
public:
    ...
    inline void reset( const BarStep& source_barstep_ )
    {
        ...
        _current_interval = APPDEF_BarStep::trans_duration2clocks( source_barstep_.repeat_distance )-1; // -1 processing upset
        ...
        _init_duration_offset = source_barstep_.repeat_duration_offset,
        _current_note_offset = 0;
...
private:
    inline void calc_swing_swing(const BarStep&barstep_, int& delay_, int &velocity_, int& duration_ ) const
    {
...
public:
    ...
    inline bool is_not_rolling_repeat() const {
        return _init_not_rolling;
    }
    ...
    inline uint8 get_current_velocity() const {
        return _init_velocity + _current_velocity_offset;
    }

    inline uint16 get_current_duration() const
    {
        int16 duration = APPDEF_BarStep::trans_duration2clocks( _init_duration ) + _current_duration_offset;
        if( duration < 1 )
            duration = 1;
        return duration;
    }

    inline uint8 get_current_repeat() const {
...
StepProcessor::StepProcessor(Pattern&pattern_)
    : _pattern( pattern_ ),
      ...
      _init_repeats( 0 ),
      _repeat_counter( 0 ),
      _repeat_as_count_down( 0 ),

      _current_interval( 0 ),
      _init_interval( 0 ),
      _init_interval_offset( 0 ),
      _current_velocity_offset( 0 ),
      _init_velocity( 0 ),
      _init_velocity_offset( 0 ),
      _current_duration_offset( 0 ),
      _init_duration( 0 ),
      _init_duration_offset( 0 ),
      _current_note_offset( 0 ),
      _init_note_offset( 0 ),
      _init_note_up_and_down( 0 ),
      _last_note_up_and_down( false ),
      _init_repeat_force_chord_notes( 0 ),
...
public:
...
#endif
        // ABSOLUT STEP FORCE
        if( _source_bar->barstep( _step_counter ).pos_force_to_absolute_step || _tmp_is_forced_to_beat_next_step )
        {
    ...
    {
        _tmp_is_forced_to_beat_next_step = true;
    }
    ...
    inline uint8 get_current_step_id() const {
    ...
    inline bool is_current_processed_bar( uint8 bar_id_ ) const {
        return bar_id_ == _source_bar->id;
    }

    inline bool is_bar_timeout() const {
        return _is_bar_timeout;
    }
...
public:
    ...
    const OwnedArray< StepProcessor >& get_step_processors() const {
        return _step_processors;
    }

    BarStepPosProcessor& get_bar_step_position_processor() {
        return _bar_step_pos_processor;
    }

    const Bar* get_source_bar() const {
        return _source_bar;
    }
...
private:
    void process_bar_pos()
    {
        ...
        {
            process_new_bar_id(_current_bar_id,
                               _last_valid_bar_id,
                               _current_bar_repeat,
    ...
    void process_new_bar_id( uint8& from_bar_id_, int&  last_valid_bar_id_, uint8& bar_repeat_count_, Array<uint8>& process_reset_points_ )
    {
        // CHECK SOLO MODE
        ...
        else
        {
            for ( int bar_id = from_bar_id_; bar_id < SUM_BARS; ++bar_id )
            {
                ...
                if( processing_is_valid )
                {
                    ...
                    {
                        if ( Sequencer::is_valid_for_auto_chain( *maybe_new_bar ) || solo_bar_ids.size() )
                        {
                            // check if we have to go back to zero, if this reset not already processed
                            from_bar_id_ = bar_id;
...
public:
    ...
    inline void hard_reset( const Pattern*const source_pattern_ )
    {
        ...
        _bar_processor->get_bar_step_position_processor().hard_reset( &_app_instance_store->pattern.bar( _current_bar_id ) );
        _bar_processor->hard_reset( &_app_instance_store->pattern.bar( _current_bar_id ) );
    }

    /// GETTER

    inline uint8 get_current_bar_id() const {
        return _current_bar_id;
    }

    inline uint8 get_current_bar_repeat() const {
        return _current_bar_repeat;
    }
    ...
    inline bool is_repeat_at_step_already_processed( uint8 step_id_ ) const {
        return _bar_processor->get_bar_step_position_processor().is_step_reset_already_processed( step_id_ );
    }
    ...
    inline BarProcessor* get_bar_processor() {
    ...
    inline uint8 get_next_bar_id() {
        if( is_probability_data_changed() )
        {
            ...
            _probabliy_last_valid_bar_id = _last_valid_bar_id;
            process_new_bar_id(_probabliy_next_bar,
                               _probabliy_last_valid_bar_id,
                               _probabliy_running_bar_repeat,
                               _probabliy_processed_reset_point_ids);
        }
...
public:
    ...
    ~BarGroupProcessor() {
        delete _bar_processor;
    }

    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (BarGroupProcessor)
};

BarGroupProcessor::BarGroupProcessor( AppInstanceStore*const app_instance_store_, uint8 bar_group_id_ )
    : _app_instance_store( app_instance_store_ ),

      _bar_processor( new BarProcessor( app_instance_store_ ) ),

      _source_pattern( nullptr ),
      id( bar_group_id_ ),
      ...
      _probabliy_next_bar( 0 ),
      _probabliy_running_bar_repeat( 1 ),
      _probabliy_last_valid_bar_id( 0 ),
...
bool Sequencer::is_valid_for_auto_chain( const Step& step_ )
{
    return ! step_.is_mute;
}
...
void Sequencer::get_current_messages( Array< MIDIMessageWithDuration* >& messages_, uint8 group_id_ ) const
{
    BarGroupProcessor* bar_group_processor;
...
void Sequencer::get_current_cc_messages( Array< MidiMessage*>& messages_, uint8 group_id_, bool only_0_and_32_ )
{
    ...
    MidiMessage message;
    for( int cc_set_id = APPDEF_Bar::CC_SET_START_INDEX  ; cc_set_id != Bar::appdeff_t::SUM_CC_SETS ; ++cc_set_id )
    {
        ...
        if( use )
        {
            ...
            if( cc_val->enable )
            {
                ...
                message = MidiMessage::controllerEvent
                          (
...
bool Sequencer::is_step_repeat_already_processed( uint8 bar_group_id_, uint8 step_id_ ) const {
    return bar_group_processors.getUnchecked( bar_group_id_ )->is_repeat_at_step_already_processed( step_id_ );
}

int8 Sequencer::get_running_step_repeat_as_count_down( uint8 bar_group_id_, uint8 step_id_ ) const {
    return bar_group_processors.getUnchecked( bar_group_id_ )->get_running_step_repeat_as_countdown_at( step_id_ );
}
