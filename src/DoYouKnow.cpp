        ...
        if( ! dont_show_agains.contains(id) )
        {
            ...
            int result = AlertWindow::showYesNoCancelBox
                         (
                             ...
                             +String("\n\n")
                             +message.message,
                             "OK","DON'T SHOW THIS INFO AGAIN","NEVER SHOW ANY INFO AGAIN"
                         );
            if( result == 2 )
            {
...
DoYouKnow::CharPair DoYouKnow::get_message(DoYouKnow::MESSAGES id_)
{
    ...
    case ASSIGN_FILE_INFOS :
        return CharPair(
                   ...
                   "To identify your project or file for later use you can insert a description in the INFO-box at the bottom of the data manager." );
    case ASSIGN_AUDIO_FILES :
        return CharPair(
                   "AUDIO SAMPLE AND TEXT INFO FOR PROJECTS",
#if JUCE_IOS
                   "NOTE iOS: record and assign not supported yet (comes with the next update)!\n\nTo identify your project/file for later use you can assign an audio sample file or record one at the bottom of the data manager." );
#else
                   "To identify your project/file for later use you can assign an audio sample file or record one at the bottom of the data manager." );
#endif
    ...
    case EDITOR_CHORDS :
        return CharPair(
                   "THE CHORD EDITOR.",
                   "Here you can edit the 6 chords of the 5 available chord sets.\n\nThe red sliders mark the chord which is currently used by the bar which you can edit on the main interface.\nBeige sliders mark the running chord.\n\nAt the bottom you have a few presets which you can assign to the selected 'PRESET TARGET' chord.");
    case NO_MIDI_LEARN_PORT_READY :
        return CharPair(
                   "NO MIDI LEARN IN PORT READY!",
                   "To use the MIDI Learn feature: please select your MIDI controller at 'MIDI LEARN :: Receive Port (IN)' in the IO settings.");
    ...
    case USE_VST_INSTEAD_OF_AU :
        return CharPair(
                   "YOU ARE USING THE B-STEP AUDIO UNIT!",
                   "If your host support VST's, then you should use the VST version instead of the AudioUnit!\nIf you are using a host like Logic please read: http://forum.monoplugs.com/logic");
    default:
        return CharPair("","");
    }
}
...
DoYouKnow::DoYouKnow(): never_show_a_message(false) {
    BOOT (DoYouKnow);
}
