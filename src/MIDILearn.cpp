...
void MIDIInToControllerHandler::change_listeners_value(const MidiMessage& message_)
{
    ...
    const uint8* sysexdata;
    {
        ...
        else if( message_.isSysEx() && _midi_message_type == LISTEN_TO_REMOTE_SYSEX )
        {
            ...
            if( sysexdata[ARRAY_B2B_SYSEX_A] != uint8(MONOPLUGS_) || sysexdata[ARRAY_B2B_SYSEX_B] != uint8(MONOPLUGS_B_STEP) )
                return;
        }
        ...
        if( controller )
        {
			...
			controller->on_param_via_changed();
                    ...
                    else
                    {
                        ...
                        _last_listeners_value = param->set_value_unsigned( sysexdata[ARRAY_B2B_SYSEX_VALUE] );
			...
			controller->on_param_via_changed();
                        ...
                        param->in_percent() > 0.5 ?
                        param->set_value( param->min_value() )
                        :
...
void MIDIInToControllerHandler::force_feedback_refresh()
{
...
void MIDIInToControllerHandler::refresh_feedback_from_last_listener()
{
    ...
    if( controller )
    {
        ...
        if( param )
        {
            if( param->get_param_ident() == APPDEF_UIUserData::parameter_name( APPDEF_UIUserData::I_SELECTED_BAR_ID ) )
                return;
...
void MIDIInToControllerHandler::update_feedback_message(float new_value_)
{
    ...
    {
        feed_back = MidiMessage::noteOn( _listen_on_channel, _midi_controller_type, uint8(new_value_*127) );
    }
    else if( _midi_message_type == LISTEN_TO_CONTROLLER )
    {
        feed_back = MidiMessage::controllerEvent( _listen_on_channel, int(_midi_controller_type), uint8(new_value_*127) );
    }
...
bool MIDIInToControllerHandler::is_controller_listen_to_you(const MONO_Controller*const listener_) const
{
...
MIDIInToControllerHandler::MIDIInToControllerHandler(int listen_message_type_, int listen_controller_type_, int listen_on_channel_)
    : _is_feedback_new( false ),
      _last_listeners_value( -1 ),
      _midi_message_type( listen_message_type_ ),
      _midi_controller_type( listen_controller_type_ ),
      _listen_on_channel( listen_on_channel_ )
...
public:
    ...
    LearningHistory( const MONO_Controller*const controller_, const MIDIInToControllerHandler*const handler_ )
...
void MIDIInToControllerMap::register_controller2handler(MONO_Controller*const controller_, const MIDIInToControllerHandler& handler_)
{
    ++_change_counter;
...
void MIDIInToControllerMap::clean_for_mode_1_1(MONO_Controller*const controller_, const MIDIInToControllerHandler& handler_)
{
...
void MIDIInToControllerMap::clean_for_mode_N_1(MONO_Controller*const, const MIDIInToControllerHandler& handler_)
{
    MIDIInToControllerHandler* registered_handler;
    Array< MIDIInToControllerHandler* > handler_to_remove;
    for( int i = 0 ; i != midi2controller_handlers.size() ; ++i )
...
void MIDIInToControllerMap::clean_for_mode_1_N(MONO_Controller*const controller_, const MIDIInToControllerHandler&)
{
    MIDIInToControllerHandler* registered_handler;
    Array< MIDIInToControllerHandler* > handler_to_remove;
    for( int i = 0 ; i != midi2controller_handlers.size() ; ++i )
...
void MIDIInToControllerMap::remove_handler_from_history(const MIDIInToControllerHandler*const handler_)
{
    LearningHistory* history;
    for( int i = 0 ; i != _learning_history.size() ; ++i )
    {
...
void MIDIInToControllerMap::add_to_history(const MONO_Controller*const controller_, const MIDIInToControllerHandler*const handler_)
{
    LearningHistory* history;
...
void MIDIInToControllerMap::remove_handler_for( const MONO_Controller*const controller_, const MIDIInToControllerHandler& handler_ )
{
    if( controller_ )
    {
        MIDIInToControllerHandler* handler;
...
Array< const MIDIInToControllerHandler* > MIDIInToControllerMap::get_registerd_handlers2controller(const MONO_Controller*const controller_) const
{
            ...
            if( handlers.size() )
            {
                ...
                if( param )
                {
                    ...
                    for( int i = 0 ; i != handlers.size() ; ++i )
                    {
                        ...
                        if( handler )
                        {
                            if( handler->_midi_message_type == MIDIInToControllerHandler::LISTEN_TO_CONTROLLER )
                                type = "CC";
                            else if( handler->_midi_message_type == MIDIInToControllerHandler::LISTEN_TO_NOTES )
                                type = "Note";
                            else
                                type = "B2B";

                            cc2controller_element = controller_element->createNewChildElement( assign+String(i) );
                            cc2controller_element->setAttribute ( "Channel", handler->_listen_on_channel );
                            cc2controller_element->setAttribute ( "Type", type );
                            cc2controller_element->setAttribute ( "Number", handler->_midi_controller_type );
                        }
                    }
                }
            }
            ...
            if( param )
            {
                if( controller_element->hasTagName( param->get_param_short_ident() ) )
                {
                    ...
                    while (cc2controller_element != nullptr)
                    {
                        ...
                        register_controller2handler( controller, MIDIInToControllerHandler( type, number, channel ) );
...
MIDIInToControllerMap::MIDIInToControllerMap(AppInstanceStore*const app_instance_store_)
    : _app_instance_store( app_instance_store_ ),
      _learning_controller( nullptr ),
      _is_in_learning_mode( false ),
      _change_counter( 0 ),
      learn_mode( _1_1 )
{}
