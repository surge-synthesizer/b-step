...
enum
{
    SINGLE_SHOT_MESSAGE = -99 // NOTE the livetime will be allways over and it will be removed on the next cleanup
};
...
private:
    ...
    public:
        ...
        Array< MIDIMessageWithDuration* >& get_retrigger_messages()
        {
            return messages_to_trigger;
        }

        RetriggerPair( MIDIMessageWithDuration*const ident_message_, MIDIMessageWithDuration*const retrigger_message_ )
    ...
    public:
        inline void add_new( MIDIMessageWithDuration*const duration_message_, MIDIMessageWithDuration*const retrigger_message_ ) {
        ...
        inline Array< MIDIMessageWithDuration* > pull_retrigger_messages( MIDIMessageWithDuration*const duration_message_ ) {
...
public:
    ...
    inline void count_down_duration()
    {
        ...
        for( int i = 0 ; i != _duration_messages.size() ; ++i )
        {
            ...
            {
                *duration_message->message = MidiMessage::noteOff
                                             (
                ...
                Array< MIDIMessageWithDuration* > retrigger_messages = retrigger_stack.pull_retrigger_messages( duration_message );
    ...
    inline const Array< MidiMessage* >& get_over_messages() const
    {
        return _over_messages;
    }
    inline void set_same_messages_to_timeover( MIDIMessageWithDuration*const duration_message_, int playback_mode_ )
    {
        ...
        // --> just add not a same not again in polyphone and only add one in monoplhon, highest or lowest
        ...
        for( int i = 0 ; i != _duration_messages.size() ; ++i )
        {
            ...
            if( playback_mode_ == APPDEF_ProcessorUserData::PLAYBACK_MODE_POLYPHON_STOP_NOTES_BEFORE_PLAY_SAME
            ...
            {
                if( to_stop_duration_message->message->getChannel() == duration_message_->message->getChannel() )
                    if( to_stop_duration_message->message->getNoteNumber() == duration_message_->message->getNoteNumber() )
                        do_action = true;
            }
            ...
            if( do_action )
            {
                ...
                if( playback_mode_ == APPDEF_ProcessorUserData::PLAYBACK_MODE_POLYPHON_RETRIGGER
                        || playback_mode_ == APPDEF_ProcessorUserData::PLAYBACK_MODE_MONOPHON_RETRIGGER )
                {
                    int new_duration = to_stop_duration_message->duration - duration_message_->duration;
                    if( new_duration > 0 )
                    {
                        MIDIMessageWithDuration* retrigger_message = new MIDIMessageWithDuration( new MidiMessage(*to_stop_duration_message->message), new_duration );
                        retrigger_stack.add_new( duration_message_, retrigger_message );
                    }
                }
                else if( playback_mode_ == APPDEF_ProcessorUserData::PLAYBACK_MODE_POLYPHON_EXPAND_LAST
                         || playback_mode_ == APPDEF_ProcessorUserData::PLAYBACK_MODE_MONOPHON_EXPAND_LAST )
                {
                    int duration_expand = to_stop_duration_message->duration - duration_message_->duration;
                ...
                *to_stop_duration_message->message = MidiMessage::noteOff
                                                     (
                                                             ...
                                                             to_stop_duration_message->message->getNoteNumber(),
                                                             (uint8_t)0
                                                     );
...
// USE ONE SEQUENCE TO DETUNE ANOTHER ONE
    ...
    void handleNoteOn (MidiKeyboardState *source, int midiChannel, int midiNoteNumber, float velocity) override
    {
        ...
        {
            //_app_instance_store->sequencer.set_string_offset( pressed_keys%4, (64-midiNoteNumber)*-1 );
            ...
            //if( ! _app_instance_store->audio_processor->is_mute )
            {
                ...
                for( int group_id = 0 ; group_id != BAR_GROUPS+MIDI_OUT_B ; ++group_id )
                {
                    ...
                    {   //if( ! _app_instance_store->audio_processor->is_mute ) {
                        if( port->is_open() )
                        {
                            Array< MIDIMessageWithDuration* >& precalculated_messages = running_midi_messages_per_group->_ready_messages;

                            // FIFO CC, PC, NOTES
                            // PC's
                            // bool pc_and_cc_is_to_send = ! feeded_PC_CC_ports.contains( port->get_dev_index() );
                            ...
                            {
                                _sequencer.get_current_cc_messages( pc_and_cc_messages, sequence_group_id, true );
                                for( int i = 0 ; i != pc_and_cc_messages.size() ; ++i )
                                {
                                    ...
                                    precalculated_messages.add( new MIDIMessageWithDuration( pc_and_cc_messages.getUnchecked(i), SINGLE_SHOT_MESSAGE ) );
                                }
                            ...
                            // if( pc_and_cc_is_to_send )
                            {
                                _sequencer.get_current_pc_messages( pc_and_cc_messages, sequence_group_id );
                                for( int i = 0 ; i != pc_and_cc_messages.size() ; ++i )
                                {
                                    MidiMessage* message = pc_and_cc_messages.getUnchecked(i);
                                    message->setChannel( _audio_processor.get_channel(group_id) );
                            ...
                            //if( pressed_keys )
                            {
                                _sequencer.get_current_messages( duration_messages, sequence_group_id );
                                for( int i = 0 ; i != duration_messages.size() ; ++i )
                                {
                                    MIDIMessageWithDuration* duration_message = duration_messages.getUnchecked(i);
                                    duration_message->message->setChannel( _audio_processor.get_channel(group_id) );
                                    duration_message->message->setNoteNumber( duration_message->message->getNoteNumber() );
#ifdef DEMO
                                    // DEMO OVER HANDLING
                                    ...
                                    running_midi_messages_per_group->set_same_messages_to_timeover( duration_message, _app_instance_store->audio_processor->note_playback_mode );
                            ...
                            // if( pc_and_cc_is_to_send )
                            {
                                _sequencer.get_current_cc_messages( pc_and_cc_messages, sequence_group_id, false );
                                for( int i = 0 ; i != pc_and_cc_messages.size() ; ++i )
                                {
                                    MidiMessage* message = pc_and_cc_messages.getUnchecked(i);
                                    message->setChannel( _audio_processor.get_channel(group_id) );
...
private:
    ...
    inline void process_output( bool only_send_stop_messages = false )
    {
        ...
        for( int group_id = 0 ; group_id != BAR_GROUPS+MIDI_OUT_B ; ++group_id )
        {
            ...
            if( port->is_open() )
            {
                const Array< MidiMessage* >& stop_messages = running_midi_messages_per_group->get_over_messages();
            ...
            // SEND NEW MESSAGES
            Array< MIDIMessageWithDuration* >& new_messages = running_midi_messages_per_group->_ready_messages;
            if( port->is_open() )
            {
                MIDIMessageWithDuration* duration_message;
                for( int i = 0 ; i != new_messages.size() ; ++i )
                {
                    ...
                    /*
                    // CC HACK
                    *duration_message->message = MidiMessage::contyrollerEvent( duration_message->message->getChannel(), _app_instance_store->audio_processor->CCommand, duration_message->message->getNoteNumber() );
                    */
    ...
    void send_sync_message_to_all_ports( const MidiMessage& message_ )
    {
        ...
        for( int i = 0 ; i != BAR_GROUPS+MIDI_OUT_B ; ++i )
        {
            ...
            if( output_guard.is_port_valid_for_sending(port) )
            {
                ...
                Array< MIDIMessageWithDuration* >& precalculated_messages = running_midi_messages_per_group->_ready_messages;
                precalculated_messages.add( new MIDIMessageWithDuration( new MidiMessage( message_ ), SINGLE_SHOT_MESSAGE ) );
            }
        }
#endif
    }
...
public:
    void send_midi_controller_messages_manually()
    {
        timerCallback();
    }
private:
    ...
    void timerCallback()
    {
        ...
        // LAUNCHPADS
        send_launchpad_messages( _app_instance_store->launchpad_1, _app_instance_store->midi_io_handler.pad_1_out );
        send_launchpad_messages( _app_instance_store->launchpad_2, _app_instance_store->midi_io_handler.pad_2_out );
        ...
        if( _app_instance_store->midi_io_handler.midi_learn_out.is_enabled() ) {
            if( _app_instance_store->midi_io_handler.midi_learn_out.is_open() )
            {
                ...
                if( sysex_messages.size() > 1 )
                {
                    ...
                    sysex_bulk_message = MidiMessage::createSysExMessage( sysex_bulk_data, sizeof(sysex_bulk_data) );
                    _midi_io_handler.midi_learn_out.send_message( sysex_bulk_message );
                }
    ...
    void send_launchpad_messages( Launchpad& launchpad_, MidiOutputObject& midi_out_, bool shutdown_ = false )
    {
    ...
    {
        send_launchpad_messages( _app_instance_store->launchpad_1, _app_instance_store->midi_io_handler.pad_1_out, true );
        send_launchpad_messages( _app_instance_store->launchpad_2, _app_instance_store->midi_io_handler.pad_2_out, true );
    }
...
MessageProcessor::MessageProcessor( AppInstanceStore*const app_instance_store_ )
    : is_playing( false ),
      ...
      clock_message( MidiMessage::midiClock() ),
      start_message( MidiMessage::midiStart() ),
      stop_message( MidiMessage::midiStop() ),
      continue_message( MidiMessage::midiContinue() ),

      _midi_io_handler( app_instance_store_->midi_io_handler ),
      _audio_processor( *app_instance_store_->audio_processor ),
      _sequencer( app_instance_store_->sequencer ),
...
public:
    void generate_clock_callbacks(const AudioPlayHead::CurrentPositionInfo &lastPosInfo,
                                  const MidiBuffer*const midiBuffer,
                                  ...
                                  const double sampleRate )
    {
        ...
        if ( (lastPosInfo.isPlaying || lastPosInfo.isRecording /*||  it can loop but not playing */ ) )
        {
            ...
            {
                USER_OUT( LOG_VST_TRANSPORT_EVENTS, "in::generate_clock_callbacks @@@ ", "changeSpeed", "", "" );
                _last_speed_factor = speed_factor;
                _listener->on_vst_pos_jumped( time_in_samples );
            }

            double clocksPerSample = double(lastPosInfo.bpm*24.f/speed_factor)/double(sampleRate*60.f);
            ...
            {
                clock = floor(clocksPerSample*double(syncSamplePos))+1; // +1 for future processing
        ...
        else
        {
            if (_wasPlaying)
            {
                ...
                USER_OUT( LOG_VST_TRANSPORT_EVENTS,
                          "in::generate_clock_callbacks @@@ wasRunning --- INFO::isPlaying; INFO::isRecording; INFO::isLooping;,",
                          String(lastPosInfo.isPlaying), String(lastPosInfo.isRecording), String(lastPosInfo.isLooping) );
...
VSTClockProcessor::VSTClockProcessor( GstepAudioProcessor*const listener_ )
    :_listener( listener_ ),
     _last_absolute_clock( -999999 ),
     _wasPlaying(false),
     _last_speed_factor(0),
     _stop_positon(-999999)
{
...
// ********************************************************************************************
// ********************************************************************************************
void GstepAudioProcessor::on_new_vst_clock( int samples_delay_, int64 absolute_clock_, double sample_rate_ = 0 )
{
    USER_OUT( LOG_VST_CLOCK, "enter::on_new_vst_clock @@@ inSamples::", String( samples_delay_ ), " clocksFromZero:", String( absolute_clock_ ) );
    ...
    USER_OUT( LOG_VST_CLOCK, "exit::on_new_vst_clock @@@ inSamples::", String( samples_delay_ ), " clocksFromZero:", String( absolute_clock_ ) );
}
...
void GstepAudioProcessor::on_vst_pos_jumped( int64 pos_ )
{
    ...
    if( pos_ )
    {
        if( _current_buffer )
        {
            USER_OUT( LOG_VST_TRANSPORT_EVENTS, "in::on_vst_pos_jumped->stop_all_pending_notes @@@ pos::", String( pos_ ), "", "" );
            //_message_processor->stop_all_pending_notes();
        }

        if( pos_ > 0 )
        {
            USER_OUT( LOG_VST_TRANSPORT_EVENTS, "in::on_vst_pos_jumped->force_to_beat_at_next_clock @@@ pos::", String( pos_ ), "", "" );
            _app_instance_store.sequencer.force_to_beat_at_next_step();
        }
    }
    else
    {
        USER_OUT( LOG_VST_TRANSPORT_EVENTS, "in::on_vst_pos_jumped->on_vst_stopped @@@ pos::", String( pos_ ), "", "" );
        on_vst_stopped( pos_ );
    }
...
void GstepAudioProcessor::on_vst_stopped( int64 pos_ )
{
    ...
    if( _current_buffer )
    {
        USER_OUT( LOG_VST_TRANSPORT_EVENTS, "in::on_vst_stopped->stop_all_pending_notes @@@ pos::", String( pos_ ), "", "" );
        _message_processor->stop_all_pending_notes();
    }
    if( pos_ <= 0 )
    {
...
void GstepAudioProcessor::on_vst_continue( int64 pos_ )
{
    ...
    USER_OUT( LOG_VST_TRANSPORT_EVENTS, "enter-exit::on_vst_continue @@@ pos::", String( pos_ ), " NOTE: empty function", "" );
}
...
void GstepAudioProcessor::processBlock ( AudioSampleBuffer& buffer_, MidiBuffer& midi_messages_ )
{
...
#else
    if( _active_writer )
        _active_writer->write ((const float**)buffer_.getArrayOfReadPointers(), buffer_.getNumSamples());
    ...
    if ( getPlayHead() != 0 )
    {
        if( getPlayHead()->getCurrentPosition ( pos ) )
        {
            ...
            // OUTPUT HANDLING
            _clock->generate_clock_callbacks( pos, _current_buffer, buffer_.getNumSamples(), getSampleRate() );

            /*
                synth.addSound (new Vocoder( buffer_, getSampleRate(), buffer_.getNumSamples(), &synth ));
            //buffer_.clear();
        ...
        if( _active_sample_playback && _sample_started )
        {
            ...
            if( _sample_playback_position >= _sample_playback_length && _sample_playback_position != 0 )
            {
            ...
            {
                _active_sample_playback->read( &buffer_,
                                               0,
                                               buffer_.getNumSamples(),
                                               _sample_playback_position,
                                               true, true );
...
// ********************************************************************************************
// ********************************************************************************************
bool GstepAudioProcessor::is_playing() const {
    return _message_processor->is_playing;
}
bool GstepAudioProcessor::is_stopped() const {
    return _message_processor->is_stoped;
}
bool GstepAudioProcessor::is_paused() const {
    return _message_processor->is_paused;
}
bool GstepAudioProcessor::is_master() const {
    return _message_processor->is_master;
}

void GstepAudioProcessor::play() {
    _message_processor->handle_user_start_event();
}
void GstepAudioProcessor::stop() {
    _message_processor->handle_user_stop_event();
}

void GstepAudioProcessor::pause() {
    _message_processor->handle_user_pause_event();
}

void GstepAudioProcessor::panic() {
    _message_processor->send_panic();
}
...
template<int inside>
inline static int fits( int source_ )
{
    if( source_ >= inside )
...
template<int count_by>
inline static bool is_smaller_else_count( int quest_, int& counter )
{
...
bool GstepAudioProcessor::isOutputChannelStereoPair ( int ) const
{
    return false;
}
...
const String GstepAudioProcessor::getProgramName ( int )
{
    return "B-Step";
}

void GstepAudioProcessor::changeProgramName ( int, const String& )
{
    return;
}
...
#endif // DEMO
{
    ...
    {
        data += (
                    String("PARAMETER: ") + getParameterName(i)
                    + String(" (") + getParameterLabel(i) + String(")")
                    + String("\n")
                    + String("DEFAULT VALUE:") + String(getParameterDefaultValue(i))
                    + String("\n")
                    + String("HELP URL:") + MANUAL_URL+ String(get_automatable_parameter( i ).get_help_url() )
                    + String("\n")+ String("\n") );
    }
    OUT(data);
#endif
...
#	endif
    ...
    for (int i = 0; i != SUM_STRINGS ; ++i)
        synth.addVoice(new VocoderVoice());   // These voices will play our custom sine-wave sounds..
    */
#endif
}
