                         ...
                         const int num_samples_) noexcept
{
    ...
    for( int sid = 0 ; sid < num_samples_ ; ++sid )
    {
        ...
        // OUTPUT // PULS FALL DOWN
        {
            ...
            // OUT
            if( fm_amount )
                sample = sample*(1.0f-fm_amount) + ( (modulator_sample + sample)*0.5f )*fm_amount;
...
#define MIN_ENV_CHANGE_TIME_IN_MS 10
    samples_to_target_left = msToSamplesFast( time_sample_rate_factor_*1000 + MIN_ENV_CHANGE_TIME_IN_MS, sample_rate ) ;
    if( samples_to_target_left > 0 )
    {
...
// -----------------------------------------------------------------
NOINLINE DoubleAnalogFilter::DoubleAnalogFilter(bool create_smooth_filter) : smooth_filter( nullptr ), last_filter_type(UNKNOWN), glide_time_4_filters(0) {
...
inline void FilterProcessor::process( DataBuffer& data_buffer_, const int num_samples ) noexcept
{
    ...
    float* out_buffers[SUM_INPUTS_PER_FILTER];
    {
        for( int input_id = 0 ; input_id != SUM_INPUTS_PER_FILTER ; ++input_id )
        {
            ...
            float* tmp_input_ar_amp = data_buffer_.filter_input_env_amps.getWritePointer( buffer_channel_id );
            input_ar_amps[input_id] = tmp_input_ar_amp;

            out_buffers[input_id] = data_buffer_.filter_output_samples.getWritePointer( buffer_channel_id );

            // CALCULATE INPUTS AND ENVELOPS
            {
                FilterData::sustain_replacement_t& input_sustain = filter_data.input_sustains->get(input_id);

                const float*const osc_input_buffer = data_buffer_.osc_samples.getReadPointer(input_id);
                if( filter_data.input_holds[input_id] ) // USING SUSTAIN
                {
                    ...
                    {
                        const float* filter_before_buffer = data_buffer_.filter_output_samples.getReadPointer(input_id + SUM_INPUTS_PER_FILTER*(id-1) );
                        for( int sid = 0 ; sid != num_samples ; ++sid )
                        {
                            ...
                            else
                            {
                                ...
                                tmp_input_buffer[sid] = protection_clipping(filter_before_buffer[sid]);
                            }
                        }
...
#define MIN_CUTOFF 40.0f
    ...
    // COLLECT RESULTS
    float*const this_direct_output_buffer = data_buffer_.direct_filter_output_samples.getWritePointer(id);
    for( int i = 0 ; i != num_samples ; ++i )
    {
        // OUTPUT MIX AND DISTORTION
        {
            float amp = filter_data.output->tick_modulated( filter_data.modulate_output ? amp_mix[i] : 0 ) ;
            ...
            this_direct_output_buffer[i] = ( this_direct_output_buffer[i]*(1.0f-clipping) + soft_clipping(this_direct_output_buffer[i])*clipping );
        }
    }
    ...
    // COMPRESS
    if( id == 0 )
        compress( this_direct_output_buffer, data_buffer_.tmp_buffer_9.getWritePointer(), this_direct_output_buffer,
                  filter_data.compressor,
                  num_samples );
    else
        compress( this_direct_output_buffer, data_buffer_.tmp_buffer_9.getWritePointer(), data_buffer_.direct_filter_output_samples.getReadPointer(id-1),
                  filter_data.compressor,
                  num_samples );
    ...
    if( id == FILTER_3 )
    {
        float*const final_filters_output_buffer = data_buffer_.direct_filter_output_samples.getWritePointer();
        const float* direct_output_buffer_flt_1 = data_buffer_.direct_filter_output_samples.getReadPointer(0);
        const float* direct_output_buffer_flt_2 = data_buffer_.direct_filter_output_samples.getReadPointer(1);
        for( int sid = 0 ; sid != num_samples ; ++sid )
        {
            final_filters_output_buffer[sid] =
                (direct_output_buffer_flt_1[sid]
                 + direct_output_buffer_flt_2[sid]
...
public:
    inline void process( DataBuffer& data_buffer_, const int start_sample_final_out_, const int num_samples_ ) noexcept;
...
// -----------------------------------------------------------------
inline void FXProcessor::process( DataBuffer& data_buffer_, const int start_sample_final_out_, const int num_samples_ ) noexcept
{
    ...
    for( int sid = 0 ; sid != num_samples_ ; ++sid )
    {
        ...
        // FINAL AMP
        const float in_l_r = input_buffer[sid] * env_amp[sid]*velocity_glide.glide_tick()*synth_data.volume.tick()*2.5f;
...
#endif
        {
            step = mono_floor(steps_per_sample*double(sync_sample_pos))+1; // +1 for future processing
        }
...
void MONOVoice::render_block (AudioSampleBuffer& buffer_, int step_number_, int start_sample_, int num_samples_) {
    ...
    for( int flt_id = 0 ; flt_id != SUM_FILTERS ; ++flt_id )
        filter_envs[flt_id]->process( data_buffer->filtered_env_amps.getWritePointer(flt_id), num_samples_ );
    ...
    // UI INFORMATIONS
    for( int i = 0 ; i != SUM_OSCS ; ++i )
        synth_data.osc_datas[i]->last_modulation_value = data_buffer->lfo_amplitudes.getReadPointer(i)[num_samples_-1];
