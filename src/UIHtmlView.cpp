...
class HTMLPArser
{
    ...
    struct TagPair {
        ...
        TagPair( String start_, String end_, bool is_closed_tag_ ) : start( start_ ), end( end_ ), is_closed_tag( is_closed_tag_ ) {}
    };
    ...
    TAGS get_next_parent_tag_line( String& data_, String& result_ ) {
        ...
        while( true )
        {
            ...
            if( get_line_for( data_, _tag_li, result_ ) )
                ...
                for( int i = 1 ; i != 7 ; ++i )
                {
                    TagPair h_tag( _tag_h.start + String(i), _tag_h.end + String(i) + String(">"), false );
...
public:
    HTMLPArser( String& data_, UIHtmlView*const view_, bool parse_nav_, bool download_complete_nav_content_, bool show_content_ )
        :
        _view( view_ ),
        _show_content( show_content_ ),
        _has_live_only_content(false),
        ...
        _tag_a( "<a href", "</a>", false )
    {
        ...
        }
        {
            ...
            if( _has_live_only_content )
                _view->add_text_part( "This part has video or similar content which is only available in a webbrowser. Click \"OPEN ONLINE VERSION\" at the right-bottom to open this part in your default webbrowser.\n\n", 6, 0, _show_content, Colour(0xff919191) );
...
class NavItem : public TreeViewItem {
    ...
    String getUniqueName() const override {
        return _url.toString(false);
    }

    bool mightContainSubItems() override {
        return getNumSubItems();
    }
    ...
    void itemSelectionChanged (bool isNowSelected) override {
        _label->setColour( Label::textColourId, isNowSelected ? Colours::yellow : Colour( GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ) );
        if( isNowSelected )
            click();
    }
...
public:
    ...
    ~NavItem() {
        _html_view->items.removeFirstMatchingValue(this);
    }
...
bool UIHtmlView::open_url( const URL& source_, bool and_download_complete_nav_content_, bool show_content_  ) {
    CHECK_THREAD_EXIT( {OUT( "open_url"); return false;} );
    ...
    if (current_url.toString(false).fromFirstOccurrenceOf(".com/", false, false) == "" || current_url.toString(false).fromFirstOccurrenceOf(".com/", false, false) == "latest-downloads")
    ...
    if( valid_data_or_file )
    {
        ...
        if(data!="")
            HTMLPArser( data, this, ! nav_is_parsed, and_download_complete_nav_content_, show_content_ );
    ...
    {
        add_text_part( "Error, can not download: "+source_.toString(false).fromLastOccurrenceOf("/",false,false), 6, 0, true, Colour( 0xffff0000 ) );
        add_text_part( "Please try to re-download later or download the offline version.", 4, 10, true, Colour( 0xffff0000 ) );
...
bool UIHtmlView::open_question_mark_content( const URL& request_for_, bool force_redownload_ ) {
    if( ! get_manual_folder().exists() || force_redownload_ )
    {
        if( ! force_redownload_ )
        {
            ...
            String message( "You can download the embedded manual by clicking \"DOWNLOAD COMPLETE MANUAL\" at the left-bottom of this window." );
            ...
            message += "\n";
            message += "\n";
            message += "To have the latest version of the manual you can re-download the manual at any time you like.";
            message += "\n";
            message += "\n";
            add_text_part( message, 5, 10, true, Colours::white );

            message = "";
            message += "If your computer is never online you can download the manual on the forum: ";
        ...
        String ok_answer = !force_redownload_ ? "YES, DOWNLOAD NOW (recommended)." : "YES, RE-DOWNLOAD NOW.";
        if( AlertWindow::showOkCancelBox( AlertWindow::QuestionIcon,
                                          "DOWNLOAD THE MANUAL?",
                                          "Would you like to download the complete manual now? It need less than 10 megabyte on your disk.",
                                          ok_answer, "NO, NOT NOW", this ) )
        {
            ...
            if( is_online )
            {
                ...
                if( force_redownload_ )
                {
                    struct UpdateThread : public AutonomThread {
                        ...
                        UpdateThread( UIHtmlView*view_, const URL&url_to_load_after_ ) : AutonomThread( "B-Manual-Updater" ), _view(view_), _url_to_load_after(url_to_load_after_) {
            ...
            AlertWindow::showMessageBox( AlertWindow::WarningIcon,
                                         "ERROR",
                                         "Can not connect to the manual server.\nMaybe the server is down or your internet connection is broken.",
                                         "OK", this );
...
void UIHtmlView::select_item_with_url( const URL& url_ ) {
    NavItem* child;
    String look_for_url = url_.toString(false).fromFirstOccurrenceOf(".com/",false,false); // remove redirect url;
    for( int i = 0 ; i != items.size() ; ++i )
    {
        child = items.getUnchecked(i);
        String child_url = child->get_url().toString(false).fromFirstOccurrenceOf(".com/",false,false); // remove real url
...
void UIHtmlView::add_text_part( const String&text_, int height_, int white_spaces_before_,  bool show_content_, Colour col_, bool make_selectable_ ) {
    CHECK_THREAD_EXIT( {OUT( "add_text_part"); return;} );
    if(!show_content_ )
        return;
    ...
    editor->setColour (TextEditor::textColourId, (col_ == Colour(0x00000000) ? Colour ( height_ < 7 ? 0xffE57A1F : 0xffffffff) : col_ ) );
    editor->setColour (TextEditor::backgroundColourId, Colour (0x00ffffff));
    //editor->setColour (TextEditor::outlineColourId, Colour (GLOBAL_VALUE_HOLDER::get().MASTER_COLOUR));
...
void UIHtmlView::add_image( const String&url_, bool show_content_ ) {
    CHECK_THREAD_EXIT( {OUT( "add_image"); return;} );
    ...
    image_holder->setImages (false, true, true,
    ...
    image_holder->setBounds( PADDING_LEFT_RIGHT, current_height, image_holder->getWidth()-PADDING_LEFT_RIGHT, image_holder->getHeight() );
...
void UIHtmlView::add_nav_link( const String&text_, const URL& target_, int deepness_, bool and_open_it_ ) {
    CHECK_THREAD_EXIT( {OUT( "add_nav_link"); return;} );
    ...
    : UiEditor("B-Manual"),_app_instance_store(app_instance_store_)
{
    ...
    update->setButtonText (TRANS("RE-DOWNLOAD COMPLETE MANUAL"));
    update->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    update->addListener (this);

    addAndMakeVisible (forum = new TextButton (String()));
    forum->setButtonText (TRANS("CHECK THE FORUM"));
    forum->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    forum->addListener (this);

    addAndMakeVisible (label = new Label ("new label",
                                          TRANS("QUESTIONS TO THIS POINT: ")));
    ...
    mail->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    ...
    online->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    ...
    label4->setColour (Label::textColourId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR));
...
void UIHtmlView::resized()
{
    ...
    viewport->setBounds (proportionOfWidth (0.3222f), proportionOfHeight (0.0167f), proportionOfWidth (0.6222f), proportionOfHeight (0.8917f));
    treeView->setBounds (proportionOfWidth (0.0111f), proportionOfHeight (0.0167f), proportionOfWidth (0.3111f), proportionOfHeight (0.8917f));
    toolbar->setBounds (getWidth() - proportionOfWidth (0.0556f), 0, proportionOfWidth (0.0556f), proportionOfHeight (0.3333f));
    update->setBounds (proportionOfWidth (0.0222f), proportionOfHeight (0.9667f) - proportionOfHeight (0.0417f), proportionOfWidth (0.2111f), proportionOfHeight (0.0417f));
    forum->setBounds (proportionOfWidth (0.4333f), proportionOfHeight (0.9667f) - proportionOfHeight (0.0417f), proportionOfWidth (0.1444f), proportionOfHeight (0.0417f));
    label->setBounds (proportionOfWidth (0.2778f), proportionOfHeight (0.9667f) - proportionOfHeight (0.0417f), proportionOfWidth (0.1556f), proportionOfHeight (0.0417f));
    label2->setBounds (proportionOfWidth (0.5889f), proportionOfHeight (0.9667f) - proportionOfHeight (0.0417f), proportionOfWidth (0.0333f), proportionOfHeight (0.0417f));
    mail->setBounds (proportionOfWidth (0.6333f), proportionOfHeight (0.9667f) - proportionOfHeight (0.0417f), proportionOfWidth (0.1444f), proportionOfHeight (0.0417f));
    label3->setBounds (proportionOfWidth (0.7889f), proportionOfHeight (0.9667f) - proportionOfHeight (0.0417f), proportionOfWidth (0.0333f), proportionOfHeight (0.0417f));
    online->setBounds (proportionOfWidth (0.9778f) - proportionOfWidth (0.1444f), proportionOfHeight (0.9667f) - proportionOfHeight (0.0417f), proportionOfWidth (0.1444f), proportionOfHeight (0.0417f));
    label4->setBounds (proportionOfWidth (0.2444f), proportionOfHeight (0.9667f) - proportionOfHeight (0.0417f), proportionOfWidth (0.0333f), proportionOfHeight (0.0417f));
    tree_view_dragger->setBounds (proportionOfWidth (0.0111f), proportionOfHeight (0.0167f), proportionOfWidth (0.3111f), proportionOfHeight (0.8917f));
    viewport_dragger->setBounds (proportionOfWidth (0.3222f), proportionOfHeight (0.0167f), proportionOfWidth (0.6222f), proportionOfHeight (0.8917f));
