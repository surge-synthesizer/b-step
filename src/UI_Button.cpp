...
UiButton::UiButton()
    : is_mouse_down(false), is_simple_drag_valid(false), is_multi_drag_valid(false),
      was_dragging(false), drop_source(nullptr), _foreground_color_1(0x00000000),
      _foreground_color_2(0x00000000), _outline_color(0x00000000)
{
...
void UiButton::mouseDown(const juce::MouseEvent &e_)
{
    USER_OUT(LOG_BUTTON_EVENTS, "mouseDown @@@ ",
             ...
             _controller ? (_controller->get_parameter()
                                ? _controller->get_parameter()->get_param_unique_ident()
                                : "")
                         : "",
             "");
    ...
    USER_OUT(LOG_BUTTON_EVENTS, "mouseDownFuEnd @@@ ",
             _controller ? _controller->get_controller_name() : "unknown",
             _controller ? (_controller->get_parameter()
                                ? _controller->get_parameter()->get_param_unique_ident()
                                : "")
...
void UiButton::timerCallback()
{
    ...
    if (!is_simple_drag_valid)
    {
        if (_controller)
        {
            ...
            USER_OUT(LOG_BUTTON_EVENTS, "onLongMouseDown @@@ ",
                     _controller ? _controller->get_controller_name() : "unknown",
                     _controller ? (_controller->get_parameter()
                                        ? _controller->get_parameter()->get_param_unique_ident()
                                        : "")
        ...
        USER_OUT(LOG_BUTTON_EVENTS, "isSimpleDragValid @@@ ",
                 _controller ? _controller->get_controller_name() : "unknown",
                 _controller ? (_controller->get_parameter()
                                    ? _controller->get_parameter()->get_param_unique_ident()
                                    : "")
...
void UiButton::mouseUp(const juce::MouseEvent &e_)
{
    ...
    USER_OUT(LOG_BUTTON_EVENTS, "mouseUpFuEnd @@@ ",
             _controller ? _controller->get_controller_name() : "unknown",
             _controller ? (_controller->get_parameter()
                                ? _controller->get_parameter()->get_param_unique_ident()
                                : "")
...
void UiButton::mouseDrag(const juce::MouseEvent &e_)
{
    ...
    if (_controller)
    {
        ...
        if (isTimerRunning())
        {
            if (e_.getDistanceFromDragStartX() >
                    this->getHeight() * GLOBAL_VALUE_HOLDER::getInstance()->MULTIDRAG_SENSITIVITY ||
                e_.getDistanceFromDragStartY() >
                    this->getHeight() * GLOBAL_VALUE_HOLDER::getInstance()->MULTIDRAG_SENSITIVITY ||
                e_.getDistanceFromDragStartX() <
                    (this->getHeight() *
                     GLOBAL_VALUE_HOLDER::getInstance()->MULTIDRAG_SENSITIVITY) *
                        -1 ||
                e_.getDistanceFromDragStartY() <
                    (this->getHeight() *
                     GLOBAL_VALUE_HOLDER::getInstance()->MULTIDRAG_SENSITIVITY) *
                        -1)
            {
                USER_OUT(LOG_BUTTON_EVENTS, "enableSimpleDrag @@@ ",
                         _controller ? _controller->get_controller_name() : "unknown",
                         _controller ? (_controller->get_parameter()
                                            ? _controller->get_parameter()->get_param_unique_ident()
                                            : "")
        ...
        if (!was_dragging)
        {
            if (is_simple_drag_valid)
            {
                if (_controller->are_you_simple_dragable())
                {
                    if (e_.getDistanceFromDragStartX() >
                            this->getHeight() *
                                GLOBAL_VALUE_HOLDER::getInstance()->SIMPLEDRAG_SENSITIVITY ||
                        e_.getDistanceFromDragStartY() >
                            this->getHeight() *
                                GLOBAL_VALUE_HOLDER::getInstance()->SIMPLEDRAG_SENSITIVITY ||
                        e_.getDistanceFromDragStartX() <
                        ...
                        e_.getDistanceFromDragStartY() <
                            (this->getHeight() *
                             GLOBAL_VALUE_HOLDER::getInstance()->SIMPLEDRAG_SENSITIVITY) *
                                -1)
                    {
                        USER_OUT(LOG_BUTTON_EVENTS, "startDrag @@@ ",
                                 _controller ? _controller->get_controller_name() : "unknown",
                                 _controller
                                     ? (_controller->get_parameter()
                                            ? _controller->get_parameter()->get_param_unique_ident()
                                            : "")
                                     : "",
                                 "");
                        ...
                        drop_source =
                            std::unique_ptr<juce::Component>(_controller->get_dragNdrop_source());
                        startDragging(_controller->get_dragNdrop_ident(), drop_source.get(),
            ...
            else if (is_multi_drag_valid)
            {
                ...
                {
                    USER_OUT(LOG_BUTTON_EVENTS, "startMultiD @@@ ",
                             _controller ? _controller->get_controller_name() : "unknown",
                             _controller
                                 ? (_controller->get_parameter()
                                        ? _controller->get_parameter()->get_param_unique_ident()
                    ...
                    if (_controller->are_you_simple_dragable())
                    {
                        drop_source =
                            std::unique_ptr<juce::Component>(_controller->get_dragNdrop_source());
                        startDragging(_controller->get_multi_dragNdrop_ident(), drop_source.get(),
                                      _model->get_drag_image(img), true);
                    }
                    else
    ...
    USER_OUT(LOG_BUTTON_EVENTS, "mouseDragFuEnd @@@ ",
             _controller ? _controller->get_controller_name() : "unknown",
             _controller ? (_controller->get_parameter()
                                ? _controller->get_parameter()->get_param_unique_ident()
                                : "")
...
void UiButton::mouseDoubleClick(const juce::MouseEvent &e_)
{
    ...
    USER_OUT(LOG_BUTTON_EVENTS, "mouseDoubleClickFuEnd @@@ ",
             _controller ? _controller->get_controller_name() : "unknown",
             _controller ? (_controller->get_parameter()
                                ? _controller->get_parameter()->get_param_unique_ident()
                                : "")
...
void UiButton::mouseWheelMove(const juce::MouseEvent &e_, const juce::MouseWheelDetails &wheel_)
{
    ...
    USER_OUT(LOG_BUTTON_EVENTS, "mouseWheelMoveFuEnd @@@ ",
             _controller ? _controller->get_controller_name() : "unknown",
             _controller ? (_controller->get_parameter()
                                ? _controller->get_parameter()->get_param_unique_ident()
                                : "")
...
void UiButton::itemDropped(const juce::DragAndDropTarget::SourceDetails &sd_)
{
    ...
    USER_OUT(LOG_BUTTON_EVENTS, "itemDroppedFuEnd @@@ ",
             _controller ? _controller->get_controller_name() : "unknown",
             _controller ? (_controller->get_parameter()
                                ? _controller->get_parameter()->get_param_unique_ident()
                                : "")
...
{
    USER_OUT(LOG_BUTTON_EVENTS, "clicked @@@ ",
             _controller ? _controller->get_controller_name() : "unknown",
             _controller ? (_controller->get_parameter()
                                ? _controller->get_parameter()->get_param_unique_ident()
                                : "")
...
void UiButton::clicked()
{
    ...
    if (_controller)
        _controller->on_clicked();
    USER_OUT(LOG_BUTTON_EVENTS, "clickedFuEnd @@@ ",
             _controller ? _controller->get_controller_name() : "unknown",
             _controller ? (_controller->get_parameter()
                                ? _controller->get_parameter()->get_param_unique_ident()
                                : "")
...
void UiButton::resized(int w_, int h_) { this->setBounds(0, 0, w_, h_); }
