...
UISlider::UISlider()
    : slider(new SliderWrapper(this)),
      is_mouse_down( false ),
      _slider_fill_color( 0x00000000 ),
      _slider_outline_color( 0x00000000 ),
      _slider_knob_color( 0x00000000 ),
      _last_painted_value( -999 )
{
...
{
    if( isTimerRunning() && getTimerInterval() == GLOBAL_VALUE_HOLDER::getInstance()->LONG_MOUSE_DOWN_INTERVAL )
    {
        if( e_.getDistanceFromDragStartX() > getHeight() * GLOBAL_VALUE_HOLDER::getInstance()->MULTIDRAG_SENSITIVITY
                || e_.getDistanceFromDragStartY() > getHeight() * GLOBAL_VALUE_HOLDER::getInstance()->MULTIDRAG_SENSITIVITY
                || e_.getDistanceFromDragStartX() < (getHeight() * GLOBAL_VALUE_HOLDER::getInstance()->MULTIDRAG_SENSITIVITY)*-1
                || e_.getDistanceFromDragStartY() < (getHeight() * GLOBAL_VALUE_HOLDER::getInstance()->MULTIDRAG_SENSITIVITY)*-1 )
        {
            stopTimer();
            Slider::mouseDrag(e_);
        }
...
void SliderWrapper::mouseDown( const MouseEvent& e_ )
{
    ...
    if( GLOBAL_VALUE_HOLDER::getInstance()->MULTIDRAG_ENABLE )
    {
        if( e_.mods == ModifierKeys::rightButtonModifier && GLOBAL_VALUE_HOLDER::getInstance()->MULTIDRAG_AT_RIGHT_MOUSE )
            startTimer( 1 );
        else if( ! GLOBAL_VALUE_HOLDER::getInstance()->MULTIDRAG_AT_RIGHT_MOUSE && _owner->use_long_mouse_down() )
...
{
    if( getTimerInterval() == GLOBAL_VALUE_HOLDER::getInstance()->LONG_MOUSE_DOWN_INTERVAL || getTimerInterval() == 1 )
    {
...
void UISlider::setTextBoxStyle(Slider::TextEntryBoxPosition box_pos_, bool is_read_only, int text_box_width, int text_box_height)
{
    slider->setTextBoxStyle ( box_pos_, is_read_only, text_box_width, text_box_height );
}

void UISlider::setSliderStyle(Slider::SliderStyle style_)
{
...
Component* UISlider::get_top_level_component()
{
    return this;
}

void* UISlider::get_top_level_impl()
{
    return this;
}
