...
const Image &UITool::get_curser_image() { return _curser_image; }
...
void UIRubber::paint(Graphics &g_)
{
    drawable->drawWithin(g_,
                         Rectangle<float>(proportionOfWidth(0.1), proportionOfHeight(0.1),
...
{
    drawable =
        DrawableImage::createFromImageData(BinaryData::rubber_svg, BinaryData::rubber_svgSize)
            .release();
}
...
{
    drawable =
        DrawableImage::createFromImageData(BinaryData::question_svg, BinaryData::question_svgSize)
            .release();
}
...
void UIClipboard::mouseDrag(const MouseEvent &e_)
{
    ...
    _curser_image =
        this->createComponentSnapshot(this->getLocalBounds()).convertedToFormat(Image::ARGB);
...
void UIClipboard::itemDropped(const SourceDetails &dragSourceDetails_)
{
    if (dragSourceDetails_.description.toString().compare(BAR_DRAG_N_DROP_IDENT) == 0)
    {
        ...
        _bar_copy_clipboard.stored_bar_source_ident =
            String("Snapshot of Bar ").operator+=(source_bar.id + 1) += String(" - click to edit");
    }
    else if (dragSourceDetails_.sourceComponent.get()->getName().compare("TlQ") == 0)
    {
...
UIClipboard::UIClipboard(AppInstanceStore *app_instance_store_,
                         GstepAudioProcessorEditor *main_window_, uint8 id_)
...
{
    if (id_ == 0)
        drawable = DrawableImage::createFromImageData(BinaryData::clipboard_1_svg,
                                                      BinaryData::clipboard_1_svgSize)
                       .release();
    else if (id_ == 1)
        drawable = DrawableImage::createFromImageData(BinaryData::clipboard_2_svg,
                                                      BinaryData::clipboard_2_svgSize)
                       .release();
    else if (id_ == 2)
        drawable = DrawableImage::createFromImageData(BinaryData::clipboard_3_svg,
                                                      BinaryData::clipboard_3_svgSize)
                       .release();
    else if (id_ == 3)
        drawable = DrawableImage::createFromImageData(BinaryData::clipboard_4_svg,
