    ...
    void mouseDrag(const MouseEvent &event)
    {
        ...
        // if( success )
        {
            ...
            if (morph_top_left < 0)
                ...
                morph_top_left = 1;
            ChangeParamOverTime::execute(DATA(synth_data).morhp_states[0], morph_top_left,
                                         DATA(synth_data).morph_motor_time);
            ...
            if (morph_top_right < 0)
                ...
                morph_top_right = 1;
            ChangeParamOverTime::execute(DATA(synth_data).morhp_states[1], morph_top_right,
                                         DATA(synth_data).morph_motor_time);
            ...
            if (morph_bottom_left < 0)
                ...
                morph_bottom_left = 1;
            ChangeParamOverTime::execute(DATA(synth_data).morhp_states[2], morph_bottom_left,
                                         DATA(synth_data).morph_motor_time);
            ...
            ChangeParamOverTime::execute(DATA(synth_data).morhp_states[3], morph_bottom_right,
                                         DATA(synth_data).morph_motor_time);
...
//==============================================================================
UiDragPad::UiDragPad() : left2right_state(0.5), top2bottom_state(0.5), current_position(0, 0)
{
...
#define Colour(x) ((x == 0xffff3b00) ? orange_replacement : Colour(x))
    ...
    g.drawRoundedRectangle(static_cast<float>(proportionOfWidth(0.1375f)),
                           static_cast<float>(proportionOfHeight(0.0846f)),
                           static_cast<float>(proportionOfWidth(0.7250f)),
                           static_cast<float>(proportionOfHeight(0.8308f)), 10.000f, 2.000f);

    float e_w(proportionOfWidth(0.2500f));
    float e_h(proportionOfHeight(0.1538f));
    g.setColour(colours.slider_track_colour);
    g.fillEllipse(static_cast<float>(proportionOfWidth(left2right_state)) + 1,
                  static_cast<float>(proportionOfHeight(top2bottom_state)) + 1, e_w - 2, e_h - 2);

    g.setColour(colours.bg_lines);
    g.drawEllipse(static_cast<float>(proportionOfWidth(left2right_state)) + 1,
                  static_cast<float>(proportionOfHeight(top2bottom_state)) + 1, e_w - 2, e_h - 2,
                  1.5f);
    ...
    g.fillEllipse(static_cast<float>(proportionOfWidth(0.5667f)),
                  static_cast<float>(proportionOfHeight(0.7455f)),
                  static_cast<float>(proportionOfWidth(0.2500f)),
                  static_cast<float>(proportionOfHeight(0.1538f)));

    g.setColour(Colour(0xffff3b00));
    g.drawEllipse(static_cast<float>(proportionOfWidth(0.5667f)),
                  static_cast<float>(proportionOfHeight(0.7455f)),
                  static_cast<float>(proportionOfWidth(0.2500f)),
                  static_cast<float>(proportionOfHeight(0.1538f)), 2.000f);
...
void UiDragPad::resized()
{
    ...
    track_area->setBounds(proportionOfWidth(0.1375f), proportionOfHeight(0.0846f),
                          proportionOfWidth(0.7250f), proportionOfHeight(0.8308f));
