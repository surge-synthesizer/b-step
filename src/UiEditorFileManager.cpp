...
// BAR AND GLOBAL STRING FEATURE MUTER - der schaltet ein feature über alles ab oder auf einem bar. Z.B: CC'S
/*
TODO 2.1
...
// TODO SESSION RECORDER TUTORIAL WITH COLOUR CHANGES LIKE THE COLOUR STYLER TO MARK AREAS TO EXPLAIN
...
// check how audiobus add the images and do the same(eliminies.. wie auch immer auch gleich mit einbinden)
// if you remove audio and assign a new one does not work
// if you create a file or folder, it will be useful to select all by default
...
protected:
    SubThreadOfFimemanager(AppInstanceStore*const app_instance_store_) : _app_instance_store(app_instance_store_) {
        was_disabled = true;
        if( FILEMANAGER_PTR )
        {
...
class PresetItem;
static void add_folder_create_options( PresetItem*const parent_, const File& folder_, const char* file_extension, int file_color_ );
static void fill_folder_view( PresetItem*const parent_, const File& folder_, const char* file_extension, int file_color_, bool add_options_in_root_ );
struct PresetItem : public TreeViewItem, Button::Listener, public FileViewOwner, public Component
{
    ...
    class FileAndFolderRenamer_Policy : UiFileViewListener
    {
        ...
        void on_text_changed(String& text_) {
            ...
            File renamed_file = _file.getParentDirectory().getChildFile( File::createLegalPathName(text_) + file_extension  );
            ...
            else
            {
                if( success )
                {
                    if( _file.moveFileTo ( renamed_file ) )
                    {
                        ...
                        {
                            File new_audio_file = renamed_file.withFileExtension( _owner_item->_audio_file.getFileExtension() );
    ...
    public:
        FileAndFolderRenamer_Policy( AppInstanceStore*app_instance_store_, PresetItem* owner_item_)
            : _app_instance_store( app_instance_store_ ),
              _owner_item( owner_item_ ),
              _file( owner_item_->_file ),
              _old_title( _owner_item->_name )
        {
        ...
        void on_text_changed(String& text_) {
            ...
            if( succes )
            {
                if( new_folder.createDirectory() )
                {
                    ...
                    fill_folder_view( _subfolder_item,
                                      new_folder,
                                      _parent_item->_file_extension,
                                      _parent_item->_label_color,
                                      true );
    ...
    public:
        FolderCreator_Policy( PresetItem* parent_item_ ) :
            ...
            _parent_item( parent_item_ ),
            _parent_folder(parent_item_->_file),

            _subfolder_item( nullptr )
        {
            _subfolder_item = new PresetItem( parent_item_->_app_instance_store, "New Folder", PresetItem::IS_DIR );
    ...
    class FileCreator_Policy : UiFileViewListener
    {
        ...
        void on_text_changed(String& text_) {
            ...
            _new_file_item->set_file( _parent_folder.getChildFile(File::createLegalFileName(text_)).withFileExtension( _parent_item->_file_extension ) );
            ...
            if( success )
            {
                if( _new_file_item->save(false) )
                {
                    ...
                    bool assign_additional_info =  AlertWindow::showOkCancelBox( AlertWindow::QuestionIcon,
                                                   "WOULD YOU LIKE TO SET ADDITIONAL INFO?",
    ...
    class UpdateServerFileStructure : public AutonomThread, public SubThreadOfFimemanager
    {
        ...
        void build_tree( const String& sub_parent_folders_, PresetItem*parent_item_ )
        {
            ...
            if( folder )
            {
                ...
                // READ FILES
                ScopedPointer<XmlElement> overall_files = download_xml( sub_parent_folders_, "files.xml" );
                ...
                if( files )
                {
                    ...
                    while( files )
                    {
                        ...
                        if( project != ERROR )
                        {
                            File local_tmp_file( get_local_path(sub_parent_folders_).getChildFile(project) );
                            if( ! local_tmp_file.exists() )
                            {
                                const MessageManagerLock mmLock;

                                found_sum_files++;
                                PresetItem* bproject = new PresetItem( parent_item_->_app_instance_store,
                                ...
                                bproject->set_download_url( get_remote_url(sub_parent_folders_,project) );
                                ...
                                {
                                    bproject->set_audio_file( get_local_path(sub_parent_folders_).getChildFile(audio_file) );
                                    bproject->set_stream_url( get_remote_url(sub_parent_folders_,audio_file) );
                                }
                            }
                        }
                ...
                while( child )
                {
                    ...
                    if( child_folder_name != ERROR )
                    {
                        ...
                        PresetItem* subfolder_item = new PresetItem( parent_item_->_app_instance_store, child_folder_name, PresetItem::IS_DOWNLOAD_DIR );
        ...
        {
            XmlElement* project_folders_overview_memory = get_remote_url(subfolders_,xml_name_).readEntireXmlStream().release();
        ...
        void run() override
        {
            ...
            {
                info = "Sorry, something was going wrong. We could not receive new stuff. Maybe the server is offline or your network connection is broken.";
                name = "ERROR";
                SHOW_ERROR_NOTIFICATION();
            }
            else
            ...
            if( info != "" )
            {
                ...
                PresetItem* item = new PresetItem( _root_download_item->_app_instance_store, name, PresetItem::IS_UNDEFINED );
    ...
    public:
        ...
        UpdateServerFileStructure( AppInstanceStore*app_instance_store_, PresetItem* presets_downloadable_ )
            : AutonomThread( "B-Download-Refresher" ),
              SubThreadOfFimemanager( app_instance_store_ ),
              _app_instance_store( app_instance_store_ ),
              _root_download_item( presets_downloadable_ ),
              local_download_folder( get_downloads_folder() ),
              has_downloaded_something( false ),
              found_sum_files( 0 )
        {
            is_working = true;
...
private:
    ...
    bool mightContainSubItems() override {
        return ( _type == IS_DIR ) ? true : false;
    }
    ...
    void paintHorizontalConnectingLine (Graphics & g, const Line< float > &line_ ) override {
        ...
        case IS_CREATE_PROJECT :
            _drawable = Drawable::createFromImageData (BinaryData::new_svg, BinaryData::new_svgSize).release();
            break;
        case IS_CREATE_FOLDER :
            _drawable = Drawable::createFromImageData (BinaryData::foldernew_svg, BinaryData::foldernew_svgSize).release();
            break;
        case IS_DOWNLOAD :
            _drawable = Drawable::createFromImageData (BinaryData::eye_svg, BinaryData::eye_svgSize).release();
            break;
        default :
            _drawable = nullptr;
        }
        ...
        if (_drawable)
        {
            Rectangle< float > area = Rectangle< float >( line_.getStartX(), line_.getStartY(),line_.getEndX()-line_.getStartX(), line_.getEndY()-line_.getStartY() );
            _drawable->replaceColour( Colour( 0xffff3b00 ), Colour( GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ) );
            _drawable->drawWithin (g,
                                   Rectangle<float>(area.getX()+area.getWidth(),
                                                    0.2*getItemHeight(),
    ...
    void paintOpenCloseButton (Graphics & g, const Rectangle< float > &area, Colour, bool /*isMouseOver*/) override {
        ...
        case IS_DOWNLOAD_DIR :
            ;
        case IS_DIR :
            _drawable = Drawable::createFromImageData (BinaryData::folder2_svg, BinaryData::folder2_svgSize).release();
            if (_drawable)
            {
                _drawable->replaceColour( Colour( 0xffff3b00 ), Colour( GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ) );
                _drawable->drawWithin (g,
...
public:
    ...
    PresetItem::TYPE get_type() const {
        return _type;
    }
    void set_color( uint32 color_ ) {
        _label_color = color_;
    }

    // INFORMATION
    void set_additional_info( const String& text_ ) {
        _info = text_;
    }

    // DATA / FILES / PROJECTS (XML)

    void set_writeprotect( bool status_ ) {
        _is_writprotect = status_;
    }
    ...
    void set_download_url( const URL& file_url_ ) {
        _file_url = file_url_;
    }
    bool is_already_on_disk() const {
        ...
        if( _type == IS_FILE )
        {
            ...
            if( _player )
            {
                ...
                for( int i = 0 ; i != extensions.size() ; ++i )
                    if( set_audio_file( _file.getParentDirectory().getChildFile( _file.getFileNameWithoutExtension()+extensions.getReference(i) ) ) )
                        return;
            }
        }
        _has_audio = false;
    ...
    void set_file_extension( const char* file_extension_ ) {
        _file_extension = file_extension_;
    }
    ...
    String& get_file_info( String& comment_ ) const {
        comment_ = ((_is_writprotect || ! FILEMANAGER_PTR->_is_in_write_mode) ? NO_FILEINFO_SET_READ_ONLY : NO_FILEINFO_SET);
        if( _info != "" )
            comment_ = _info;
        ...
        {
            ScopedPointer<XmlElement> xml = XmlDocument( _file.getChildFile("directory.info") ).getDocumentElement().release();
            if( xml )
                comment_ = xml->getStringAttribute( "COMMENT", comment_ );
        }
    ...
    void remove_audio()
    {
        ...
        bool remove_it =  AlertWindow::showOkCancelBox( AlertWindow::QuestionIcon,
                          "REMOVE AUDIO?",
            ...
            else if( _last_playback_item != this )
            {
                ...
                if( _has_audio )
                {
                    ...
                    if( _audio_data && _audio_data_size )
                        AUDIO_PLAYER_PTR->loadFileIntoTransport( new MemoryInputStream( _audio_data, _audio_data_size, false ) );
                    // FROM URL STREAM
                    else if( _audio_url != URL(ERROR) )
#ifdef B_STEP_STANDALONE
#ifndef LINUX
                        AUDIO_PLAYER_PTR->loadFileIntoTransport( _audio_url.createInputStream( false, nullptr, nullptr, String(), 4000 ) )
#endif
			;
#else
                        ;
    ...
    void assign_new_audio( const File& new_audio_file_ )
    {
        class AudioAssigner : public UiDualAudioMessageListener, public SubThreadOfFimemanager
        {
            ...
            void on_ok() override
            {
                ...
                File new_audio_file_at_new_destination = _project_file.withFileExtension( _new_audio_file.getFileExtension() );
            ...
            void on_chancel() override
            {
                SHOW_CANCEL_NOTIFICATION();
            };

            const File& get_new_audio_file() override {
                return _new_audio_file;
            }
            const File& get_old_audio_file() override {
                return _old_audio_file;
            }

        public:
            AudioAssigner( AppInstanceStore*app_instance_store_,
                           PresetItem*const owner_,
                           const File& new_audio_file_,
                           const File& old_audio_file_,
                           const File& project_file_ )
                : SubThreadOfFimemanager( app_instance_store_ ),
                  _app_instance_store( app_instance_store_ ),
                  _owner( owner_ ),
                  _new_audio_file( new_audio_file_ ),
                  _old_audio_file( old_audio_file_ ),
        ...
        new UiDualAudioMessage(  _app_instance_store,
                                 new AudioAssigner(
                                     _app_instance_store,
                                     this,
                                     new_audio_file_,
    ...
    bool read_xmlfile_from_memory_stream() const {
        ...
        // PROJECTS
        if( _file_extension == APPDEFF::project_file_extension ) {
            ScopedPointer<XmlElement> parsed = XmlDocument::parse( MemoryInputStream ( _data, _data_size, false ).readEntireStreamAsString()).release();
            if( parsed )
            ...
            if( _app_instance_store->last_loaded_project.existsAsFile() )
            {
                ...
                if( _file != _app_instance_store->last_loaded_project && _file.exists() )
                {
                    if( AlertWindow::showOkCancelBox( AlertWindow::WarningIcon,
                                                      ...
                                                      + String("\nWould you like to replace by ") + _app_instance_store->last_loaded_project.getFileNameWithoutExtension() + String(" ?"),
                                                      "REPLACE", "KEEP IT", FILEMANAGER_PTR ) )
                    {
                        do_it = true;
                        SESSION_ERROR_LOG( "OVERWRITE EXISTING FILE -> true " + _file.getFileNameWithoutExtension() );
                    }
                    else
                    {
                        do_it = false;
                        SESSION_ERROR_LOG( "OVERWRITE EXISTING FILE -> false " + _file.getFileNameWithoutExtension() );
                    }
                }
            }
        }
    ...
    void download_all_childs( bool start_new_thread = true ) {
        struct DownloadExecuter : public AutonomThread {
            ...
            void exec() {
                ...
                {
                    PresetItem* child = reinterpret_cast< PresetItem* >( _parent_folder->getSubItem(i) );
                    if( child )
                    {
                        is_done = false;

                        String x_of = String(i) + String("/") + String( _parent_folder->getNumSubItems() );
            ...
            DownloadExecuter( PresetItem*const parent_folder_, bool was_master_download_ )
                :
                AutonomThread("B-Download-Pool"),
                _parent_folder( parent_folder_ ),
    ...
    void handle_download_from_server( bool volatile* autoclose_on_done_ = nullptr ) {
        ...
        class DownloadCallback :
            public UiDualDownloadListener,
            public SubThreadOfFimemanager
        {
        ...
        public:
            DownloadCallback( PresetItem*item_,
                              Downloader*const data_downloader_,
                              bool volatile* autoclose_on_done_ )
                :
                UiDualDownloadListener( data_downloader_ ),
                SubThreadOfFimemanager( item_->_app_instance_store ),
                _item(item_),
                _app_instance_store( item_->_app_instance_store ),
                _autoclose_on_done( autoclose_on_done_ ),
                should_set_down_on_kill(false)
            {}
        ...
        if( ! _file.existsAsFile() )
        {
            if( _file_url != URL(ERROR) )
            {
                ...
                if( _file.getParentDirectory().createDirectory() )
                {
                    ...
                    if( should_write_folder_info )
                        if( getParentItem() )
                            write_folder_info( _file.getParentDirectory(), reinterpret_cast< PresetItem* >( getParentItem() )->_info );

                    // DATA DOWNLOAD
                    download_callback = new DownloadCallback( this,
                            new Downloader(_file_url,_file),
                            autoclose_on_done_ );
                    ...
                    if( ! _audio_file.existsAsFile() )
                        if( _audio_url != URL(ERROR) )
                            ...
                            {
                                download_callback->set_audio_downloader( new Downloader( _audio_url,_audio_file ) );
                            }
                }
            }
            if( download_callback )
            {
                ...
                UiDualDownload* ui_download = new UiDualDownload(  _app_instance_store, download_callback );
    ...
    bool save( bool auto_close_ = true ) {
        ...
        case IS_FILE :
            ...
            if( success && FILEMANAGER_PTR->_was_a_reader_request && auto_close_ )
            {
                ...
                UiEditorFileManager* reader = new UiEditorFileManager( _app_instance_store, READ_MODE, FILEMANAGER_PTR->_view_type );
    ...
    void remove() {
        ...
        switch( _type ) {
        case IS_DIR :
            _file.findChildFiles( files, File::findFilesAndDirectories | File::ignoreHiddenFiles, true );
            if( AlertWindow::showOkCancelBox( AlertWindow::WarningIcon,
                                              "DELETE A WHOLE FOLDER???",
                                              _file.getFileNameWithoutExtension() + String(" contains ") + String( files.size() )
                                              ...
                                              "DELETE", "KEEP IT", FILEMANAGER_PTR ) )
            {
                    ...
                    && this->getParentItem() )
                {
                    ...
                    SESSION_ERROR_LOG( "DELETE FOLDER -> success " + _file.getFileNameWithoutExtension() );
                }
                else
                {
                    SHOW_ERROR_NOTIFICATION();
                    GLOBAL_ERROR_LOG( "DELETE FOLDER -> error " + _file.getFileNameWithoutExtension() );
                }
            }
            else
            {
        ...
        case IS_FILE :
            if( AlertWindow::showOkCancelBox( AlertWindow::WarningIcon,
                                              "DELETE FILE???",
                                              String("Delete file: ") + _file.getFileNameWithoutExtension() + String(" ?"),
                                              "DELETE", "KEEP IT", FILEMANAGER_PTR ) )
            {
                    ...
                    && this->getParentItem() )
                {
                    ...
                    this->getParentItem()->removeSubItem(this->getIndexInParent(),true);
                    SHOW_CUSTOM_NOTIFICATION("REMOVED",1);
                    SESSION_ERROR_LOG( "DELETE FILE -> success " + _file.getFileNameWithoutExtension() );
                }
                else
                {
                    SHOW_ERROR_NOTIFICATION();
...
static void add_folder_create_options( PresetItem*const parent_, const File& folder_, const char* file_extension, int file_color_ )
{
    ...
    PresetItem* item = new PresetItem( parent_->_app_instance_store, ident, PresetItem::IS_CREATE_PROJECT );
    parent_->addSubItem(item);
    item->set_color( file_color_ );
    item->set_file(folder_);
    item->set_file_extension( file_extension );

    item = new PresetItem( parent_->_app_instance_store, "CREATE NEW SUBFOLDER", PresetItem::IS_CREATE_FOLDER );
    parent_->addSubItem(item);
...
static void fill_folder_view( PresetItem*const parent_, const File& folder_, const char* file_extension, int file_color_, bool add_options_in_root_ )
{
    ...
    for( int i = 0 ; i != b_projects.size() ; ++i )
    {
        ...
        {
            PresetItem* bproject = new PresetItem( parent_->_app_instance_store, file.getFileNameWithoutExtension(), PresetItem::IS_FILE );
        ...
        else if( file.isDirectory() )
        {
            ...
            {
                PresetItem* subfolder = new PresetItem( parent_->_app_instance_store, file.getFileName(), PresetItem::IS_DIR );
                parent_->addSubItem(subfolder);
                fill_folder_view( subfolder, b_projects.getUnchecked(i), file_extension, file_color_, add_options_in_root_ );
...
#define ADD_BINARY_PRESET( parent_, name_, binary_ ) \
    preset_item =  new PresetItem( preset_folder_->_app_instance_store, name_, PresetItem::IS_BINARY_FILE ); \
...
#define ADD_BINARY_PRESET_WITH_AUDIO( parent_, name_, binary_, audio_ ) \
    ...
    PresetItem* arpeggios = new PresetItem( preset_folder_->_app_instance_store, "ARPEGGIOS", PresetItem::IS_DIR );
    ...
    PresetItem* progressions = new PresetItem( preset_folder_->_app_instance_store, "PROGRESSION", PresetItem::IS_DIR );
    progressions->set_additional_info( "PROGRESSION PRESETS\n\nA collection of simple projects which will show you some nice chord progressions." );
    ...
    ADD_BINARY_PRESET_WITH_AUDIO( progressions, "1225 (soft shuffle)", _1225softSuffle_b2proj, _1225softSuffle_ogg );
    ...
    ADD_BINARY_PRESET_WITH_AUDIO( progressions, "1564 (shuffle)", _1564shuffle_b2proj, _1564shuffle_ogg );
    ADD_BINARY_PRESET_WITH_AUDIO( progressions, "1645 (shuffle)", _1645shuffle_b2proj, _1645shuffle_ogg );
    ADD_BINARY_PRESET_WITH_AUDIO( progressions, "4516", _4516_b2proj, _4516_ogg );
    ADD_BINARY_PRESET_WITH_AUDIO( progressions, "6415 (hard shuffle)", _6415hardShuffle_b2proj, _6415hardShuffle_ogg );
    ...
    ADD_BINARY_PRESET_WITH_AUDIO( progressions, "QDown (soft shuffle)", qdownsoftShuffle_b2proj, qdownsoftShuffle_ogg );

    // DRUMS

    PresetItem* drums = new PresetItem( preset_folder_->_app_instance_store, "DRUMS", PresetItem::IS_DIR );
    drums->set_additional_info( "DRUM PRESETS\n\nA collection of drum presets.\nNOTE: Drum Synthesizer often receive it's MIDI notes at MIDI channel 10" );
    ...
    ADD_BINARY_PRESET_WITH_AUDIO( drums, "Techno - D (ratcheting)", technoDratcheting_b2proj, technoDratcheting_ogg );
    ADD_BINARY_PRESET_WITH_AUDIO( drums, "Techno - E (ratcheting)", technoEratcheting_b2proj, technoEratcheting_ogg );

    // MIXED

    PresetItem* drums_and_melodic = new PresetItem( preset_folder_->_app_instance_store, "DRUMS & MELODIC", PresetItem::IS_DIR );
    drums_and_melodic->set_additional_info( "DRUM PRESETS\n\nA collection of drum presets.\nNOTE: Drum Synthesizer often receive it's MIDI notes at MIDI channel 10" );
    drums_and_melodic->set_writeprotect(true);
    preset_folder_->addSubItem( drums_and_melodic );

    ADD_BINARY_PRESET_WITH_AUDIO( drums_and_melodic, "Drum'n'Bass with Arpeggio - A", drumNbasewitharpeggio_b2proj, drumNbasewitharpeggio_ogg );
    ADD_BINARY_PRESET_WITH_AUDIO( drums_and_melodic, "Drum'n'Bass with Arpeggio - B", drumNbasewitharpeggioB_b2proj, drumNbasewitharpeggioB_ogg );
...
#define ADD_CHORD_PRESET_WITH_AUDIO( binary_, name_, audio_ ) \
    ...
    ADD_CHORD_PRESET_WITH_AUDIO( EABABBdirtyMajor_b2cset, "EAB-ABB-dirtyMajor", EABABBdirtyMinor_ogg );
...
void create_download_items( PresetItem*parent_, String name_, String folder_name_, const char* file_extension_, uint32 colour_, bool is_in_write_mode )
{
    PresetItem* downloads_item = new PresetItem( parent_->_app_instance_store, String("PRESETS (Download)"), PresetItem::IS_DIR );
    parent_->addSubItem( downloads_item );
    downloads_item->set_writeprotect(true);
    downloads_item->set_additional_info( String("Downloaded " ) + name_ + String(" from the online repository (read/writeable).") );

    fill_folder_view( downloads_item, get_downloads_folder().getChildFile(folder_name_), file_extension_, colour_, false );

    if( ! is_in_write_mode )
    {
        PresetItem* item = new PresetItem( parent_->_app_instance_store, String("REFRESH DOWNLOADABLE ")+name_.toUpperCase(), PresetItem::IS_DOWNLOAD );
        parent_->addSubItem(item);
        item->set_additional_info( String("Click this entry to watch for new ") + name_ + String(" in the online repository.") );
        item->set_file_extension( file_extension_ );
    }
}

PresetItem* create_fill_subroot_folders( PresetItem*root_, String name_, String folder_, String description_, const char* file_extension_, uint32 colour_, bool is_in_write_mode )
{
    PresetItem* subfolder_item = new PresetItem( root_->_app_instance_store, name_.toUpperCase(), PresetItem::IS_DIR );
    root_->addSubItem( subfolder_item );
    root_->set_file(File(name_));
    subfolder_item->set_additional_info( description_ );
    subfolder_item->set_writeprotect(true);

    PresetItem* user_folder_item = new PresetItem( root_->_app_instance_store, String("MY ")+name_.toUpperCase(), PresetItem::IS_DIR );
    subfolder_item->addSubItem( user_folder_item );
    user_folder_item->set_additional_info( "A read/writeable folder which contains your " + name_ + String(".") );
    user_folder_item->set_writeprotect(true);

    fill_folder_view( user_folder_item, get_app_folder().getChildFile(folder_), file_extension_, colour_, is_in_write_mode );
...
void UiEditorFileManager::build_init_tree_view()
{
    ...
    PresetItem* root = new PresetItem( _app_instance_store, "B-STEP SEQUENCER DATA", PresetItem::IS_DIR );
    treeView->setRootItem(root);
    root->set_writeprotect(true);
    treeView->setRootItemVisible(false);

    {
        PresetItem* project_folder_item = create_fill_subroot_folders( root,
                                          "Projects",
                                          "projects",
                                          "A project file contains a complete B-Step Project.\nBut no settings (like Multidrag Sensitivity) or Snapshots.",
                                          APPDEFF::project_file_extension,
                                          FONT_DEFAULT_COLOUR,
                                          _is_in_write_mode );
        ...
        {
            PresetItem* presets_item = new PresetItem( _app_instance_store, "PRESETS (Built-In)", PresetItem::IS_DIR );
            project_folder_item->addSubItem( presets_item );
            presets_item->set_additional_info( "A readable folder which contains the factory projects." );
        ...
        create_download_items( project_folder_item, "Projects", "projects", APPDEFF::project_file_extension, FONT_DEFAULT_COLOUR, _is_in_write_mode );
    ...
    {
        PresetItem* chordsets_folder_item = create_fill_subroot_folders( root,
                                            "Chordsets",
                                            "chordsets",
                                            "A chordset file contains a set of 6 chords which you can assign to one of your 5 ChordSets per Project.",
                                            APPDEFF::chordset_file_extension,
                                            0xff00ff9a,
                                            _is_in_write_mode );

        if( ! _is_in_write_mode )
        {
            PresetItem* chordsets_presets_item = new PresetItem( _app_instance_store, "PRESETS (Built-In)", PresetItem::IS_DIR );
            chordsets_presets_item->set_additional_info( "A readable folder which contains the factory chordsets." );
        ...
        create_download_items( chordsets_folder_item, "Chordsets", "chordsets", APPDEFF::chordset_file_extension, 0xff00ff9a, _is_in_write_mode );
    ...
    {
        PresetItem* snapshots_folder_item = create_fill_subroot_folders( root,
                                            "Snapshots",
                                            "snapshots",
                                            "A snapshots file contains the data of one bar which you can copy from or to the clipboard.",
                                            APPDEFF::snapshot_file_extension,
                                            0xff00a4ff,
                                            _is_in_write_mode );

        create_download_items( snapshots_folder_item, "Snapshots", "snapshots", APPDEFF::snapshot_file_extension, 0xff00a4ff, _is_in_write_mode );
    ...
    {
        PresetItem* colour_folder_item = create_fill_subroot_folders( root,
                                         "Colour Themes",
                                         "colours",
                                         "A colour theme file stores a colour setup which you can design with the colour styler.",
                                         APPDEFF::colortheme_file_extension,
                                         0xff9400ff,
                                         _is_in_write_mode );

        create_download_items( colour_folder_item, "Colour Themes", "colours", APPDEFF::colortheme_file_extension, 0xff9400ff, _is_in_write_mode );
    ...
    {
        PresetItem* mappings_folder_item = create_fill_subroot_folders( root,
                                           "Controller Mappings",
                                           "mappings",
                                           "A controller mapping file stores complete MIDI learn patches.",
                                           APPDEFF::mapping_file_extension,
                                           0xfffffc00,
                                           _is_in_write_mode );

        create_download_items( mappings_folder_item, "Controller Mappings", "mappings", APPDEFF::mapping_file_extension, 0xfffffc00, _is_in_write_mode );
        mappings_folder_item->setOpen(_view_type==VIEW_TYPE::MAPPINGS);
    }

    // RESTORE OPEN
    {
        ScopedPointer<XmlElement> xml = XmlDocument( File(get_view_restore_file( _view_type )) ).getDocumentElement().release();
        if( xml )
    ...
    struct SearchAndSelect {
        SearchAndSelect( PresetItem*const parent_item_, File& search_for_file_, PresetItem*item_found_ ) {
...
void UiEditorFileManager::timerCallback()
{
    ...
    if( item )
    {
        ...
        // FILE CHANGE OPTIONS
        enabled = ( type == PresetItem::IS_FILE || type == PresetItem::IS_DIR ) && item->is_writable();
...
#ifndef IS_MOBILE_APP
        open->setEnabled(enabled || type == PresetItem::IS_DOWNLOAD_FILE || type == PresetItem::IS_DOWNLOAD_DIR || ( type == PresetItem::IS_DIR && item->get_file().getFullPathName() != "" ) );
#else
        open->setEnabled(enabled || type == PresetItem::IS_DOWNLOAD_FILE || type == PresetItem::IS_DOWNLOAD_DIR );
#endif
    ...
    toggleButton->setToggleState( _app_instance_store->editor_config.autoplay_sample_audio, dontSendNotification );
    ...
    if( ! info_is_in_edit_mode ) {
        ...
        if( selected_item )
        {
            ...
            String file = ( String(">> ") + selected_item->get_file().getFullPathName().fromFirstOccurrenceOf(get_app_folder().getFullPathName(),false,false) );

            if( last_selected_item != selected_item )
            {
                last_selected_item = selected_item;

                //path_view->setColour(Label::textColourId, Colour (last_selected_item->_label_color));
...
{
    OUT("lost");
    Component* component_under_mouse = Desktop::getInstance().getMainMouseSource().getComponentUnderMouse();
...
//==============================================================================
UiEditorFileManager::UiEditorFileManager (AppInstanceStore* const app_instance_store_, bool write_mode_on_, VIEW_TYPE view_type_)
    : UiEditor("B-FileManager"),_app_instance_store(app_instance_store_),_is_in_write_mode(write_mode_on_), _view_type(view_type_)
{
    addAndMakeVisible (label5 = new Label (String(),
                                           TRANS("AUTO PLAY SAMPLE AUDIO ON SELECT")));
    label5->setFont (Font ("Oswald", 15.00f, Font::plain));
    label5->setJustificationType (Justification::centredRight);
    label5->setEditable (false, false, false);
    label5->setColour (Label::textColourId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    ...
    open->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    ...
    import->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    ...
    info->setColour (TextEditor::textColourId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    info->setColour (TextEditor::backgroundColourId, Colour (0xff161616));
    info->setColour (TextEditor::highlightColourId, Colours::yellow);
    info->setColour (TextEditor::outlineColourId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    ...
    addAndMakeVisible (label2 = new Label (String(),
                                           TRANS("NOTES / COMMENTS")));
    label2->setFont (Font ("Oswald", 20.00f, Font::plain));
    label2->setJustificationType (Justification::centredLeft);
    label2->setEditable (false, false, false);
    label2->setColour (Label::textColourId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    label2->setColour (TextEditor::textColourId, Colours::black);
    label2->setColour (TextEditor::backgroundColourId, Colour (0x00000000));

    addAndMakeVisible (path_view = new Label (String(),
            TRANS("Label Text")));
    ...
    path_view->setColour (Label::textColourId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    ...
    play->setColour (TextButton::textColourOnId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    play->setColour (TextButton::textColourOffId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    ...
    label3->setColour (Label::textColourId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    ...
    record->setColour (TextButton::textColourOnId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    record->setColour (TextButton::textColourOffId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));

    addAndMakeVisible (delete_audio = new TextButton (String()));
    delete_audio->setButtonText (TRANS("REMOVE"));
    delete_audio->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    delete_audio->addListener (this);
    delete_audio->setColour (TextButton::buttonColourId, Colours::black);
    delete_audio->setColour (TextButton::textColourOnId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    delete_audio->setColour (TextButton::textColourOffId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));

    addAndMakeVisible (assign = new TextButton (String()));
    assign->setButtonText (TRANS("ASSIGN"));
    assign->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    assign->addListener (this);
    assign->setColour (TextButton::buttonColourId, Colours::black);
    assign->setColour (TextButton::textColourOnId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    assign->setColour (TextButton::textColourOffId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    ...
    rename->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    ...
    delete_file->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    ...
    toggleButton->setColour (ToggleButton::textColourId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));

    addAndMakeVisible (show_audio_dyk = new TextButton (String()));
    show_audio_dyk->setButtonText (TRANS("?"));
    show_audio_dyk->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    show_audio_dyk->addListener (this);
    show_audio_dyk->setColour (TextButton::buttonColourId, Colours::black);
    show_audio_dyk->setColour (TextButton::textColourOnId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    show_audio_dyk->setColour (TextButton::textColourOffId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));

    addAndMakeVisible (show_info_dyk = new TextButton (String()));
    show_info_dyk->setButtonText (TRANS("?"));
    show_info_dyk->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    show_info_dyk->addListener (this);
    show_info_dyk->setColour (TextButton::buttonColourId, Colours::black);
    show_info_dyk->setColour (TextButton::textColourOnId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    show_info_dyk->setColour (TextButton::textColourOffId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));

    addAndMakeVisible (save = new TextButton (String()));
    save->setButtonText (TRANS("REPLACE"));
    save->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    ...
    treeView->setColour (TreeView::linesColourId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));

    addAndMakeVisible (export_ = new TextButton (String()));
    export_->setButtonText (TRANS("EXPORT"));
    export_->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    ...
    show_new_stuff->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    show_new_stuff->addListener (this);
    show_new_stuff->setColour (TextButton::buttonColourId, Colours::black);
    show_new_stuff->setColour (TextButton::textColourOnId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    show_new_stuff->setColour (TextButton::textColourOffId, Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));

    addAndMakeVisible (confirm_text_changes = new TextButton (String()));
    confirm_text_changes->setButtonText (TRANS("SET (OR PRESS RETURN)"));
    confirm_text_changes->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    ...
    cancel_text_changes->setConnectedEdges (Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom);
    ...
    should_refresh_all = false;
    tmp_audio = File::getSpecialLocation( File::tempDirectory ).getChildFile( "B-Step-Audio-Record.ogg" );
    ...
    if( _view_type == VIEW_TYPE::PROJECTS )
        ...
        {
            int feedback = AlertWindow::showOkCancelBox( AlertWindow::QuestionIcon,
                           "SAVE CURRENT PROJECT FIRST?",
                           "Would you like to save: " + _app_instance_store->last_loaded_project.getFileNameWithoutExtension() + String(" ?"),
    ...
    if( _is_in_write_mode )
    {
        ...
        if( _audio_recorder->get_selected_device_id() > -1 )
            audio_source_devices->setSelectedItemIndex( _audio_recorder->get_selected_device_id(), dontSendNotification );
#endif
    }
    else
    {
...
void UiEditorFileManager::paint (Graphics& g)
{
    ...
    g.setColour (Colour (GLOBAL_VALUE_HOLDER::getInstance()->MASTER_COLOUR ));
    g.fillRect (proportionOfWidth (0.0333f), proportionOfHeight (0.6786f), proportionOfWidth (0.9333f), 1);
...
void UiEditorFileManager::resized()
{
    ...
    label5->setBounds (proportionOfWidth (0.7333f), proportionOfHeight (0.6071f), proportionOfWidth (0.2333f), proportionOfHeight (0.0536f));
    info_animation->setBounds (proportionOfWidth (0.4833f), -10, proportionOfWidth (0.4167f), proportionOfHeight (0.1607f));
    open->setBounds (proportionOfWidth (0.7167f) - proportionOfWidth (0.2167f), proportionOfHeight (0.6964f), proportionOfWidth (0.2167f), proportionOfHeight (0.0446f));
    import->setBounds (proportionOfWidth (0.9667f) - proportionOfWidth (0.2167f), proportionOfHeight (0.6964f), proportionOfWidth (0.2167f), proportionOfHeight (0.0446f));
    toolbar->setBounds (getWidth() - proportionOfWidth (0.0833f), 0, proportionOfWidth (0.0833f), proportionOfHeight (0.3571f));
    info->setBounds (proportionOfWidth (0.0333f), proportionOfHeight (0.7589f), proportionOfWidth (0.9333f), proportionOfHeight (0.1429f));
    label->setBounds (proportionOfWidth (0.0333f), proportionOfHeight (0.0179f), proportionOfWidth (0.4500f), proportionOfHeight (0.0786f));
    label2->setBounds (proportionOfWidth (0.0833f), proportionOfHeight (0.6964f), proportionOfWidth (0.4567f), proportionOfHeight (0.0429f));
    path_view->setBounds (proportionOfWidth (0.0333f), proportionOfHeight (0.6161f), proportionOfWidth (0.5600f), proportionOfHeight (0.0429f));
    play->setBounds (proportionOfWidth (0.3167f) - proportionOfWidth (0.0833f), proportionOfHeight (0.9196f), proportionOfWidth (0.0833f), proportionOfHeight (0.0446f));
    label3->setBounds (proportionOfWidth (0.0833f), proportionOfHeight (0.9196f), proportionOfWidth (0.1500f), proportionOfHeight (0.0429f));
    record->setBounds (proportionOfWidth (0.6000f) - proportionOfWidth (0.1833f), proportionOfHeight (0.9196f), proportionOfWidth (0.1833f), proportionOfHeight (0.0446f));
    delete_audio->setBounds (proportionOfWidth (0.9667f) - proportionOfWidth (0.0833f), proportionOfHeight (0.9196f), proportionOfWidth (0.0833f), proportionOfHeight (0.0446f));
    assign->setBounds (proportionOfWidth (0.4167f) - proportionOfWidth (0.0833f), proportionOfHeight (0.9196f), proportionOfWidth (0.0833f), proportionOfHeight (0.0446f));
    audio_source_devices->setBounds (proportionOfWidth (0.8500f) - proportionOfWidth (0.2500f), proportionOfHeight (0.9196f), proportionOfWidth (0.2500f), proportionOfHeight (0.0446f));
    rename->setBounds (proportionOfWidth (0.8500f) - proportionOfWidth (0.1000f), proportionOfHeight (0.6964f), proportionOfWidth (0.1000f), proportionOfHeight (0.0446f));
    delete_file->setBounds (proportionOfWidth (0.9667f) - proportionOfWidth (0.1000f), proportionOfHeight (0.6964f), proportionOfWidth (0.1000f), proportionOfHeight (0.0446f));
    toggleButton->setBounds (proportionOfWidth (0.6833f), proportionOfHeight (0.6071f), proportionOfWidth (0.2833f), proportionOfHeight (0.0536f));
    show_audio_dyk->setBounds (proportionOfWidth (0.0733f) - proportionOfWidth (0.0417f), proportionOfHeight (0.9196f), proportionOfWidth (0.0417f), proportionOfHeight (0.0446f));
    show_info_dyk->setBounds (proportionOfWidth (0.0733f) - proportionOfWidth (0.0417f), proportionOfHeight (0.6964f), proportionOfWidth (0.0417f), proportionOfHeight (0.0446f));
    save->setBounds (proportionOfWidth (0.5833f) - proportionOfWidth (0.1167f), proportionOfHeight (0.6964f), proportionOfWidth (0.1167f), proportionOfHeight (0.0446f));
    treeView->setBounds (proportionOfWidth (0.0333f), proportionOfHeight (0.1071f), proportionOfWidth (0.9333f), proportionOfHeight (0.4964f));
    export_->setBounds (proportionOfWidth (0.7167f) - proportionOfWidth (0.1167f), proportionOfHeight (0.6964f), proportionOfWidth (0.1167f), proportionOfHeight (0.0446f));
    finger_dragger->setBounds (proportionOfWidth (0.0333f), proportionOfHeight (0.1071f), proportionOfWidth (0.9000f), proportionOfHeight (0.5000f));
    show_new_stuff->setBounds (proportionOfWidth (0.9667f) - proportionOfWidth (0.4667f), proportionOfHeight (0.9196f), proportionOfWidth (0.4667f), proportionOfHeight (0.0446f));
    confirm_text_changes->setBounds (proportionOfWidth (0.7167f) - proportionOfWidth (0.2333f), proportionOfHeight (0.6964f), proportionOfWidth (0.2333f), proportionOfHeight (0.0446f));
    cancel_text_changes->setBounds (proportionOfWidth (0.9667f) - proportionOfWidth (0.2333f), proportionOfHeight (0.6964f), proportionOfWidth (0.2333f), proportionOfHeight (0.0446f));
...
void UiEditorFileManager::buttonClicked (Button* buttonThatWasClicked)
{
    ...
    else if (buttonThatWasClicked == import)
    {
            ...
            bool on_ok() override
            {
                ...
                if( xml )
                {
                    ...
                    if( ! success )
                    {
                        AlertWindow::showMessageBox( AlertWindow::WarningIcon,
                                                     "IMPORT ERROR!!!",
                                                     "Your imported data does not match to any supported B-Step data file.\nPlease check if you have paste the whole exported data.",
        ...
        public:
            TextImporter( AppInstanceStore*const app_instance_store_, const String& title_, const String& default_text_ )
        ...
        new UiTextImExport( _app_instance_store, new TextImporter( _app_instance_store, "IMPORTER", "" ) );
#endif //DEMO
        ...
        if( _audio_recorder )
        {
            if( ! _audio_recorder->isRecording() )
            {
                if( get_selected_item() )
                {
                    ...
                    {
                        AlertWindow::showMessageBox(AlertWindow::WarningIcon,"RECORDING ERROR","Audio Recorder not ready.","OK", FILEMANAGER_PTR );
    ...
    else if (buttonThatWasClicked == assign)
    {
        ...
        struct LoadPolicy
        {
            ...
            inline void put_file_to_target( const File& file_ ) {
                if( owner->AUDIO_PLAYER_PTR )
                {
                    ...
                    else
                    {
                        AlertWindow::showMessageBox (AlertWindow::WarningIcon,
                                                     ...
                                                     file_.getFileExtension() + " file format is not supported.\nSupported audio formats are: "
                                                     + owner->AUDIO_PLAYER_PTR->get_supported_audio_formats(),"DAMN", owner->FILEMANAGER_PTR );
                ...
                {
                    AlertWindow::showMessageBox(AlertWindow::WarningIcon,"PLAYBACK ERROR","Audio Player not ready.","OK", owner->FILEMANAGER_PTR );
            ...
            LoadPolicy( UiEditorFileManager*const owner_, const File& project_file_ ) : project_file( project_file_ ), owner( owner_ ) {}
        };
        ...
        else
        {
            AlertWindow::showMessageBox (AlertWindow::WarningIcon,
                                         "ERROR",
                                         "You can only assign a audio file to a B-Step project file.\nPlease select a project file first.","OK", this );
        }
    ...
    else if (buttonThatWasClicked == export_)
    {
        ...
        public:
            TextExporter( AppInstanceStore*const app_instance_store_, const String& title_, const String& default_text_ )
        ...
        if( get_selected_item() )
        {
            ...
            if( data == "" )
                ...
                new UiTextImExport( _app_instance_store, new TextExporter( _app_instance_store, "EXPORTER", data ) );
        }
    ...
    else if (buttonThatWasClicked == show_new_stuff)
    {
        ...
        if( _app_instance_store->editor )
        {
            ...
            _app_instance_store->editor_config.manual_editor->try_open_url( MANUAL_URL+"latest-downloads" );
        }
...
void UiEditorFileManager::comboBoxChanged (ComboBox* comboBoxThatHasChanged)
{
    ...
    if (comboBoxThatHasChanged == audio_source_devices)
    {
        ...
        if( _audio_recorder )
        {
            ...
            audio_source_devices->setText( _audio_recorder->get_selected_device_name(), dontSendNotification );
        }
