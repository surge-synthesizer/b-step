...
struct FAttackSlConfig : public ModulationSliderConfigBase {
    ...
    String get_top_value() const override {
        float value = DATA(env_datas[id]).attack * DATA(env_datas[id]).max_attack_time * 1000 + MIN_ENV_CHANGE_TIME_IN_MS;
        if( value < 100 )
...
struct FDecaySlConfig : public ModulationSliderConfigBase {
    ...
    String get_top_value() const override {
        float value = DATA(env_datas[id]).decay * DATA(env_datas[id]).max_decay_time * 1000 + MIN_ENV_CHANGE_TIME_IN_MS;
        if( value < 0 )
            return "OFF";
        else if( value < 100 )
            return String(round01(value));
...
struct FReleaseSlConfig : public ModulationSliderConfigBase {
    ...
    String get_top_value() const override {
        float value = DATA(env_datas[id]).release * DATA(env_datas[id]).max_release_time * 1000 + MIN_ENV_CHANGE_TIME_IN_MS;
        if( value < 100 )
            return String(round01(value));
        else
            return String(round0(value));
