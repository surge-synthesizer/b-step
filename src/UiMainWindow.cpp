...
void GstepAudioProcessorEditor::open_chord_editor()
{
    if( ! _config->chord_editor )
    {
        ...
        _config->chord_editor->setBounds(width_prop*270,
                                         height_prop*20,
                                         width_prop*780,
                                         height_prop*500);
...
void GstepAudioProcessorEditor::open_midi_learn_editor()
{
    if( ! _config->midi_learn_editor )
    {
        ...
        _config->controller_mid_learn = new ControllerMIDILearn( _app_instance_store, midi_learn_focus, midi_cc_value );
...
void GstepAudioProcessorEditor::open_reader( VIEW_TYPE type_ )
{
    if( _app_instance_store->editor_config.file_manager )
        ...
        _app_instance_store->editor_config.file_manager = new UiEditorFileManager(_app_instance_store, false, type_ );
}
#ifdef DEMO
...
bool GstepAudioProcessorEditor::open_whats_window()
{
    ...
    {
        if (GLOBAL_VALUE_HOLDER::getInstance()->DONT_SHOW_AGAIN_2_1 || GLOBAL_VALUE_HOLDER::getInstance()->WHATS_NEW_WAS_UP)
            return false;
...
void GstepAudioProcessorEditor::open_writer( VIEW_TYPE type_ )
{
    ...
    if( _app_instance_store->editor_config.file_manager )
        ...
        _app_instance_store->editor_config.file_manager = new UiEditorFileManager(_app_instance_store,true, type_ );

#endif // DEMO
}

static inline void add_as_kylistener_to_all_childs( KeyListener*const listener_, Component*const parent_ )
{
    Component*comp;
...
bool GstepAudioProcessorEditor::keyPressed ( const KeyPress& key_, Component *originatingComponent_ )
{
    ...
    else if( key_.getTextDescription() == "ctrl + +" || key_.getTextDescription() == "ctrl + =" || key_.getTextDescription() == "ctrl + shift + +" )
    {
    ...
    else if( key_.getTextDescription() == "ctrl + -")
    {
        ...
        if( h > float(APPDEF_UIUserData::WINDOW_HEIGHT) / 2 && w > float(APPDEF_UIUserData::WINDOW_WIDTH) / 2 )
        {
#ifdef B_STEP_STANDALONE
            getParentComponent()->setSize( w, h );
            //setSize(w,h);
...
void GstepAudioProcessorEditor::visibilityChanged()
{
    ...
    if( isVisible() )
    {
        if( _app_instance_store->editor_config.demo_time_is_over && ! _app_instance_store->editor_config.demo_window )
            _config->demo_window = new UiEditorDemo( _app_instance_store );
    }
#endif
}
    ...
    : AudioProcessorEditor(processor_),_app_instance_store(&processor_->_app_instance_store)
{
    ...
    midi_learn_focus->setImages (false, true, false,
                                 Image(), 1.000f, Colour (0x00000000),
                                 Image(), 1.000f, Colour (0x00000000),
                                 Image(), 1.000f, Colour (0x00000000));
    addAndMakeVisible (midi_cc_value = new Label (String(),
            TRANS("n/a")));
    ...
    addAndMakeVisible( _editor_right_mainwindow = new UiEditorRightMainwindows( _app_instance_store, this ));
...
void GstepAudioProcessorEditor::paint (Graphics& g)
{
    ...
    g.fillRoundedRectangle (1.0f, 1.0f, static_cast<float> (proportionOfWidth (0.9672f)), static_cast<float> (getHeight() - 2), 10.000f);

    g.setColour (Colour (0xff1111ff));
    g.drawRoundedRectangle (1.0f, 1.0f, static_cast<float> (proportionOfWidth (0.9672f)), static_cast<float> (getHeight() - 2), 10.000f, 2.000f);
...
void GstepAudioProcessorEditor::resized()
{
    ...
    TimerLock timerlock( this );
    {
        ...
        _columns_fixed_steps.getUnchecked(0)->setBounds( width_prop*COL_0_X, height_prop*COL_0_Y,width_prop*COLUMN_WIDTH,height_prop*COLUMN_HEIGHT);
        _columns_fixed_steps.getUnchecked(1)->setBounds( width_prop*COL_1_X, height_prop*COL_1_Y,width_prop*COLUMN_WIDTH,height_prop*COLUMN_HEIGHT);
        _columns_fixed_steps.getUnchecked(2)->setBounds( width_prop*COL_2_X, height_prop*COL_2_Y,width_prop*COLUMN_WIDTH,height_prop*COLUMN_HEIGHT);
        _columns_fixed_steps.getUnchecked(3)->setBounds( width_prop*COL_3_X, height_prop*COL_3_Y,width_prop*COLUMN_WIDTH,height_prop*COLUMN_HEIGHT);
        _columns_fixed_steps.getUnchecked(4)->setBounds( width_prop*COL_4_X, height_prop*COL_4_Y,width_prop*COLUMN_WIDTH,height_prop*COLUMN_HEIGHT);

        _columns_fixed_bar.getUnchecked(0)->setBounds( width_prop*COL_11_X, height_prop*COL_11_Y,width_prop*COLUMN_WIDTH,height_prop*COLUMN_HEIGHT);
        _columns_fixed_bar.getUnchecked(1)->setBounds( width_prop*COL_12_X, height_prop*COL_12_Y,width_prop*COLUMN_WIDTH,height_prop*COLUMN_HEIGHT);

        for( int layer_id = 0 ; layer_id != 7 ; ++layer_id ) // SUM LAYERS
        {
            _columns.getUnchecked(layer_id)->getUnchecked(0)->setBounds( width_prop*COL_5_X, height_prop*COL_5_Y,width_prop*COLUMN_WIDTH,height_prop*COLUMN_HEIGHT);
            _columns.getUnchecked(layer_id)->getUnchecked(1)->setBounds( width_prop*COL_6_X, height_prop*COL_6_Y,width_prop*COLUMN_WIDTH,height_prop*COLUMN_HEIGHT);
            _columns.getUnchecked(layer_id)->getUnchecked(2)->setBounds( width_prop*COL_7_X, height_prop*COL_7_Y,width_prop*COLUMN_WIDTH,height_prop*COLUMN_HEIGHT);
            _columns.getUnchecked(layer_id)->getUnchecked(3)->setBounds( width_prop*COL_8_X, height_prop*COL_8_Y,width_prop*COLUMN_WIDTH,height_prop*COLUMN_HEIGHT);
            _columns.getUnchecked(layer_id)->getUnchecked(4)->setBounds( width_prop*COL_9_X, height_prop*COL_9_Y,width_prop*COLUMN_WIDTH,height_prop*COLUMN_HEIGHT);
            _columns.getUnchecked(layer_id)->getUnchecked(5)->setBounds( width_prop*COL_10_X, height_prop*COL_10_Y,width_prop*COLUMN_WIDTH,height_prop*COLUMN_HEIGHT);
        }

        //// MENUE BARS
        _editor_right_mainwindow->setBounds( width_prop*(1242-206-5), width_prop*10, width_prop*206, height_prop*748 );
