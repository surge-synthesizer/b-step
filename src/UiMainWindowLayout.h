...
void GstepAudioProcessorEditor::auto_resize_to_user_area()
{
    ...
    resizeLimits.setSizeLimits( float(APPDEF_UIUserData::WINDOW_WIDTH) / 2,
                                float(APPDEF_UIUserData::WINDOW_HEIGHT) / 2,
                                APPDEF_UIUserData::WINDOW_WIDTH * 20, APPDEF_UIUserData::WINDOW_HEIGHT * 20 );

    resizeLimits.setFixedAspectRatio ( float(APPDEF_UIUserData::WINDOW_WIDTH) / APPDEF_UIUserData::WINDOW_HEIGHT );
#endif
    ...
    if( is_desktop_to_small )
    {
        if( height_factor > width_factor ) {
            ...
            parent->getConstrainer()->setFixedAspectRatio(float(APPDEF_UIUserData::WINDOW_WIDTH) / (APPDEF_UIUserData::WINDOW_HEIGHT+28));
            parent->centreWithSize( width_factor*APPDEF_UIUserData::WINDOW_WIDTH*0.95, width_factor*(APPDEF_UIUserData::WINDOW_HEIGHT+28)*0.95 );
        ...
        else
        {
            ...
            int y_border = parent->getTitleBarHeight()+parent->getBorderThickness().getTopAndBottom();
            parent->getConstrainer()->setFixedAspectRatio(float(APPDEF_UIUserData::WINDOW_WIDTH) / (APPDEF_UIUserData::WINDOW_HEIGHT+28));
            parent->centreWithSize( height_factor*APPDEF_UIUserData::WINDOW_WIDTH*0.95, height_factor*(APPDEF_UIUserData::WINDOW_HEIGHT+28)*0.95 );
    ...
    else
    {
        ...
        int y_border = parent->getTitleBarHeight()+parent->getBorderThickness().getTopAndBottom();
        parent->getConstrainer()->setFixedAspectRatio(float(APPDEF_UIUserData::WINDOW_WIDTH) / (APPDEF_UIUserData::WINDOW_HEIGHT+28));
        parent->centreWithSize( width, (APPDEF_UIUserData::WINDOW_HEIGHT+28)*(1.f/APPDEF_UIUserData::WINDOW_WIDTH*width) );
...
#else
        setBounds(0,0, width ,
                  APPDEF_UIUserData::WINDOW_HEIGHT*(1.f/APPDEF_UIUserData::WINDOW_WIDTH*width) );
#endif
    }
}
...
void GstepAudioProcessorEditor::refresh_selected_bar( Array< Component* >& components_to_repaint_ )
{
    ...
    if (selected_bar_id != last_painted_bar || refresh_bar_select_paint)
    {
        ...
        {
            _last_painted_selected_bar_models.add( _columns[_config->current_layer]->getUnchecked(2)->get_column_editor()->get_model(selected_bar_id) );
            _last_painted_selected_bar_models.add(  _columns[_config->current_layer]->getUnchecked(3)->get_column_editor()->get_model(selected_bar_id) );
            _last_painted_selected_bar_models.add(  _columns[_config->current_layer]->getUnchecked(4)->get_column_editor()->get_model(selected_bar_id) );
            _last_painted_selected_bar_models.add(  _columns[_config->current_layer]->getUnchecked(5)->get_column_editor()->get_model(selected_bar_id) );
        }
        else if( _config->current_layer == 3 )
        {
            _last_painted_selected_bar_models.add(  _columns[_config->current_layer]->getUnchecked(0)->get_column_editor()->get_model(selected_bar_id) );
            _last_painted_selected_bar_models.add(  _columns[_config->current_layer]->getUnchecked(1)->get_column_editor()->get_model(selected_bar_id) );
            _last_painted_selected_bar_models.add(  _columns[_config->current_layer]->getUnchecked(2)->get_column_editor()->get_model(selected_bar_id) );
            _last_painted_selected_bar_models.add(  _columns[_config->current_layer]->getUnchecked(3)->get_column_editor()->get_model(selected_bar_id) );
            _last_painted_selected_bar_models.add(  _columns[_config->current_layer]->getUnchecked(4)->get_column_editor()->get_model(selected_bar_id) );
            _last_painted_selected_bar_models.add(  _columns[_config->current_layer]->getUnchecked(5)->get_column_editor()->get_model(selected_bar_id) );
        }
        _last_painted_selected_bar_models.add(  _columns_fixed_bar.getUnchecked(0)->get_column_editor()->get_model(selected_bar_id) );
        _last_painted_selected_bar_models.add(  _columns_fixed_bar.getUnchecked(1)->get_column_editor()->get_model(selected_bar_id) );

        // PAINT
        for( int i = 0 ; i != _last_painted_selected_bar_models.size() ; ++i )
        {
            _last_painted_selected_bar_models.getUnchecked(i)->set_background_style( _app_instance_store->style_step_area );
        }
...
public:
    ...
    void count_down_livetime() {
        --_live_time;
    }

    void reset_livetime( int live_time_ = 2 ) {
        _live_time = live_time_;
    }
    ...
    LabelPopupWithLivetime( const MONO_Controller*const controller_,
        ...
        is_top_popup( _controller->paint_popup_above() ),
        _init_value( _controller->get_value() )
    {
        /* WORKS ON IPAD?
    ...
    void set_bounds( float width_prop_, float height_prop_ ) {
        ...
        popup->setBounds( x-(width_prop_*5),
                          y - y_off,
                          w*width_prop_,
                          h*height_prop_ );
    }
...
void GstepAudioProcessorEditor::check_paint_slider_value_popup()
{
    if( ! _app_instance_store->midi_in_map.is_in_learning_mode() )
    {
        Array< MONO_Controller* > controllers_that_need_a_popup;
        {
            ...
            {
                _columns.getUnchecked(_config->current_layer)->getUnchecked(i)->get_controllers_for_paint_popup( controllers_that_need_a_popup );
            }

            if( _config->chord_editor )
                _config->chord_editor->get_controllers_for_paint_popup( controllers_that_need_a_popup );

            _editor_right_mainwindow->get_controllers_for_paint_popup( controllers_that_need_a_popup );
        }

        // CHECK IF THE POPUP STILL IS PRESENT, ELSE MAKE A NEW
        for( int i = 0 ; i != controllers_that_need_a_popup.size() ; ++i )
        {
            const MONO_Controller*const constroller_need_a_popup = controllers_that_need_a_popup.getUnchecked(i);
            ...
            {
                LabelPopupWithLivetime* popup = new LabelPopupWithLivetime( constroller_need_a_popup );
...
void GstepAudioProcessorEditor::timerCallback()
{
    ...
    for( uint8 i = 0 ; i != _columns.getUnchecked(_config->current_layer)->size() ; ++i )
    {
        _columns.getUnchecked(_config->current_layer)->getUnchecked(i)->refresh_ui( components_to_repaint );
    }
...

    {
        ...
        for( int i = 0 ; i != SUM_STRINGS ; ++i )
        {
            pattern_and_barstring_offset = (_app_instance_store->pattern.master_string_octave(i)
                                            + selected_bar.barstring(i).octave_offset)*OCTAVE_MULTIPLIER;

            int value = pattern_and_barstring_offset+base_note_value+Sequencer::get_base_note_value( chord, i );
            if( _app_instance_store->editor_config.current_chord_view == APPDEF_UIUserData::SHOW_CHORDS )
            {
                if(value >= 0 && value < 128 )
                    _columns_fixed_steps.getUnchecked(i+1)->set_text( String( MidiMessage::getMidiNoteName(_app_instance_store->sequencer.get_string_offset(i)+_app_instance_store->pattern.get_remote_offset()+pattern_and_barstring_offset+base_note_value+Sequencer::get_base_note_value( chord, i ),true,true,3) ) );
                else
            ...
            else // DRUMS
            {
                if( value >= 0 && _app_instance_store->editor_config.drum_names.size() > value )
                    _columns_fixed_steps.getUnchecked(i+1)->set_text( _app_instance_store->editor_config.drum_names.getUnchecked( value ) );
                else
                    _columns_fixed_steps.getUnchecked(i+1)->set_text("-x-");
            }
        }
    ...
    if( is_first_callback )
    {
        ...
        if( GLOBAL_VALUE_HOLDER::getInstance()->SESSION_FILE_WAS_STILL_PRESENT )
        {
            ...
            int answer = AlertWindow::showYesNoCancelBox(AlertWindow::WarningIcon,
                         "B-STEP WAS CLOSED UNEXPECTED!",
                         "Please send the logs to Monoplugs to help fixing that issue.",
                         "YES, SEND AS MAIL", "OPEN LOG FOLDER", "IGNORE",
                         this);
            ...
            if( answer == 1 )
            {
                URL(String("mailto:support@monoplugs.com?subject=B-Step-Report&body=B-Step-Report ") + ProjectInfo::versionString + String(" (") + additional_info + String( ")  \n\n\n" + GLOBAL_VALUE_HOLDER::getInstance()->COPY_OF_SESSION_FILE )).launchInDefaultBrowser();
            }
            else if( answer == 2 )
            {
	    ...
	    {
                _app_instance_store->editor_config.question_editor = new UiQuestionIsYourFriend( _app_instance_store );
...
//#ifndef DEMO
                                ...
                                {
                                    GLOBAL_VALUE_HOLDER::getInstance()->USER_ASKED_FOR_AUTO_UPDATE = true;
                                    GLOBAL_VALUE_HOLDER::getInstance()->AUTO_CHECK_UPDATES = AlertWindow::showOkCancelBox(AlertWindow::QuestionIcon,
                                            "AUTOMATICALLY CHECK FOR UPDATES?",
                                            "Would you like to atomatically check for new B-Step updates?",
                                            "YES, CHECK AUTOMATICALLY (recommended)",
                                            "NEVER",_owner);
...
//#endif
                                ...
                                {
                                    number = v_info.fromFirstOccurrenceOf("<!-- VERSION:",false,false).upToFirstOccurrenceOf(" -->",false,false).getFloatValue();
                                    minor = v_info.fromFirstOccurrenceOf("<!-- MINOR:",false,false).upToLastOccurrenceOf(" -->",false,false).getIntValue();
                                }

                                OUT( "B-VERSION: "<< number<<"."<<minor );

                                if( number > float(B_STEP_VERSION) || ( number == float(B_STEP_VERSION) && minor > B_STEP_MINOR_VERSION ) )
                                {
                                    int answer = AlertWindow::showYesNoCancelBox(AlertWindow::InfoIcon,
                                                 "VERSION " + String(number)+String(".")+String(minor)+ " IS AVAILABLE!",
                                                 "Would you like to see what's new in this version?",
                                                 "SHOW ME THAT STUFF!", "NO, NOT NOW", "DISABLE UPDATE CHECK",
                                                 _owner);

                                    if( answer == 1 )
                                    {
                                        if(!_owner-> _app_instance_store->editor_config.manual_editor )
                                            _owner->_app_instance_store->editor_config.manual_editor = new UIHtmlView( _owner->_app_instance_store );

                                        _owner->_app_instance_store->editor_config.manual_editor->try_open_url( MANUAL_URL );
                                    }
                                    else if( answer == 2 )
                                        ;// NO
                                    else
				    {
//#ifndef DEMO
                                        GLOBAL_VALUE_HOLDER::getInstance()->AUTO_CHECK_UPDATES = false;
										_owner->_app_instance_store->save_default_files();
				    }
//#endif
                                }
                        ...
                        public:
                            CheckUpdates(GstepAudioProcessorEditor*const owner_) : AutonomThread("B-UpdateChecker"),_owner(owner_) {
                                startThread(1);
                            }
                        };
...
public:
    TimerLock( Timer*const timer_ )
        : _timer( timer_ ),
          _timer_interval( timer_->getTimerInterval() ),
...
void GstepAudioProcessorEditor::init_layer_controllers_until_steps()
{
    ...
    model_column->set_controllers( _app_instance_store->controller.bar.barstring.getUnchecked(0)->step.mutes );
    UiLeftsideLabelModel* leftside_label_model = new UiLeftsideLabelModel(_app_instance_store);
    leftside_label_model->set_text( "1" );
    leftside_label_model->set_controller( _app_instance_store->controller.bar.barstring.getUnchecked(0)->octave );
    ...
    const_cast< ModelBase* >( _app_instance_store->controller.bar.barstring.getUnchecked(0)->octave->get_model() )->set_style( _app_instance_store->style_step_area_octave );

    column_wrapper = _columns_fixed_steps.getUnchecked(2);
    model_column = column_wrapper->get_column_editor();
    model_column->set_controllers( _app_instance_store->controller.bar.barstring.getUnchecked(1)->step.mutes );
    leftside_label_model = new UiLeftsideLabelModel(_app_instance_store);
    leftside_label_model->set_text( "2" );
    leftside_label_model->set_controller( _app_instance_store->controller.bar.barstring.getUnchecked(1)->octave );
    column_wrapper->set_leftside_subeditor( leftside_label_model );
    column_wrapper->set_style( _app_instance_store->style_step_area_steps );
    const_cast< ModelBase* >( _app_instance_store->controller.bar.barstring.getUnchecked(1)->octave->get_model() )->set_style( _app_instance_store->style_step_area_octave );

    column_wrapper = _columns_fixed_steps.getUnchecked(3);
    model_column = column_wrapper->get_column_editor();
    model_column->set_controllers( _app_instance_store->controller.bar.barstring.getUnchecked(2)->step.mutes );
    leftside_label_model = new UiLeftsideLabelModel(_app_instance_store);
    leftside_label_model->set_text( "3" );
    leftside_label_model->set_controller( _app_instance_store->controller.bar.barstring.getUnchecked(2)->octave );
    column_wrapper->set_leftside_subeditor( leftside_label_model );
    column_wrapper->set_style( _app_instance_store->style_step_area_steps );
    const_cast< ModelBase* >( _app_instance_store->controller.bar.barstring.getUnchecked(2)->octave->get_model() )->set_style( _app_instance_store->style_step_area_octave );

    column_wrapper = _columns_fixed_steps.getUnchecked(4);
    model_column = column_wrapper->get_column_editor();
    model_column->set_controllers( _app_instance_store->controller.bar.barstring.getUnchecked(3)->step.mutes );
    leftside_label_model = new UiLeftsideLabelModel(_app_instance_store);
    leftside_label_model->set_text( "4" );
    leftside_label_model->set_controller( _app_instance_store->controller.bar.barstring.getUnchecked(3)->octave );
    column_wrapper->set_leftside_subeditor( leftside_label_model );
    column_wrapper->set_style( _app_instance_store->style_step_area_steps );
    const_cast< ModelBase* >( _app_instance_store->controller.bar.barstring.getUnchecked(3)->octave->get_model() )->set_style( _app_instance_store->style_step_area_octave );
}
...
void GstepAudioProcessorEditor::init_layer_controllers_page()
{
    ...
    for( int layer_id = 0 ; layer_id != 7 ; ++layer_id )
    {
        for( int i = 0 ; i != _columns[layer_id]->size() ; ++i)
        {
            ...
            model_column->set_controllers(_app_instance_store->layer_controller.getReference(layer_id).getUnchecked(5+i));

            leftside_label = new UiLeftsideLabel();
            leftside_label->set_text(_app_instance_store->layer_names.getReference(layer_id).getReference(5+i) );

            column_wrapper->set_leftside_subeditor( leftside_label );
            column_wrapper->set_style( _app_instance_store->layer_styles.getReference(layer_id).getUnchecked(5+i) );
        }
    }
}
...
void GstepAudioProcessorEditor::init_layer_controllers_page_7()
{
    ...
    UiColumn16* model_column = column_wrapper->get_column_editor();
    model_column = column_wrapper->get_column_editor();
    model_column->set_controllers(_app_instance_store->layer_controller.getReference(6).getUnchecked(col));
    UiLeftsideLabel* leftside_label = new UiLeftsideLabel();
...
void GstepAudioProcessorEditor::get_selecter_area( Rectangle<int>& area_, uint8 id_ )
{
    ...
    int selecter_x = elem_x+column_x+column_wrapper_x-selecter_space; // elemens are 40, width = 50
...
void GstepAudioProcessorEditor::complex_paint (Graphics& g)
{
    ...
    g.fillRoundedRectangle (width_prop*42, 1.0f, width_prop*(APPDEF_UIUserData::WINDOW_WIDTH-42*2), static_cast<float> (getHeight() - 2), 10.000f);
    g.setColour (Colour (_app_instance_store->style_global_area->get_border_color()));
    g.drawRoundedRectangle (width_prop*42, 1.0f, width_prop*(APPDEF_UIUserData::WINDOW_WIDTH-42*2), static_cast<float> (getHeight() - 2), 10.000f, 2.000f);

    struct PaintBG
    {
        static inline void paint_rounded_rectangle(
            Graphics& g_,
            uint32 c_fore_, uint32 c_border_,
            Rectangle<int>& area_ )
        {
            // Content
            g_.setColour(Colour(c_fore_));
            g_.fillRoundedRectangle(area_.getX(),area_.getY(),area_.getWidth(),area_.getHeight(),10);

            // Border
            g_.setColour(Colour(c_border_));
            g_.drawRoundedRectangle(area_.getX(),area_.getY(),area_.getWidth(),area_.getHeight(),10,2);
        }
        static inline void paint_rectangle(
            Graphics& g_,
            uint32 c_fore_, uint32 c_border_,
            Rectangle<int>& area_ )
        {
    ...
    Rectangle<int> area;
    get_bar_area(area);
    PaintBG::paint_rounded_rectangle( g,
                                      _app_instance_store->style_bar_area->get_foreground_color(),
                                      _app_instance_store->style_bar_area->get_border_color(),
                                      area );

    // SELECTER
    get_selecter_area( area, _app_instance_store->editor_config.selected_bar_id );
    PaintBG::paint_rectangle( g,
                              _app_instance_store->style_step_area->get_foreground_color(),
                              _app_instance_store->style_step_area->get_border_color(),
                              area );
