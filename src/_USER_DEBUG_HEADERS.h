...
#	define USER_OUT( LOG_TYPE, a,b,c,d ) \
	...
	{ \
	  ...
	  if( _STORE ) \
	  { \
	    UiEditorAbout* _EDITOR_ABOUT = GLOBAL_VALUE_HOLDER::getInstance()->_app_instance_store->editor_config.about_winodow; \
	    if( _EDITOR_ABOUT ) \
	    { \
	      _EDITOR_ABOUT->print_debug( String(LOG_TYPE) + String(a) + String(b) + String(c)+ String(d) ); \
	    } \
	  } \
	} // EO LOG_TYPE
// ----------------------------------------
