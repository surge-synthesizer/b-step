...
void mono_AudioDeviceManager::open_port(const String &port_ident_name_, const String &device_name_)
{
    ...
    if (output_ident_names.contains(port_ident_name_))
    {
        ...
        else if (port_ident_name_ == "SEND_MIDI_CLOCK")
        {
            ...
            if (do_open)
                clock_output =
                    MidiOutputWrapper::open(device_index, port_ident_name_, device_name_);
        }
        else if (port_ident_name_ == "SEND_MIDI_LFO_1")
        {
            ...
            {
                lfo_outputs.set(
                    0, MidiOutputWrapper::open(device_index, port_ident_name_, device_name_));
                if (lfo_outputs[0])
                    lfo_outputs[0]->start();
            }
        }
        else if (port_ident_name_ == "SEND_MIDI_LFO_2")
        {
            ...
            {
                lfo_outputs.set(
                    1, MidiOutputWrapper::open(device_index, port_ident_name_, device_name_));
                if (lfo_outputs[1])
                    lfo_outputs[1]->start();
            }
        }
        else if (port_ident_name_ == "SEND_MIDI_LFO_3")
        {
            ...
            {
                lfo_outputs.set(
                    2, MidiOutputWrapper::open(device_index, port_ident_name_, device_name_));
                if (lfo_outputs[2])
                    lfo_outputs[2]->start();
            }
        }
        ...
        else if (port_ident_name_ == "SEND_MIDI_ADSR")
        {
            ...
            {
                adsr_outputs.set(
                    3, MidiOutputWrapper::open(device_index, port_ident_name_, device_name_));
                if (adsr_outputs[3])
    ...
    else if (input_ident_names.contains(port_ident_name_))
    {
        ...
        if (port_ident_name_ == "RECIEVE_MIDI_MAIN")
        {
            ...
            if (do_open)
                main_input =
                    MidiInputWrapper::open(device_index, port_ident_name_, device_name_, this);
        }
        else if (port_ident_name_ == "RECIEVE_CC")
        {
            cc_input = nullptr;
...
void mono_AudioDeviceManager::send_lfo_message(int lfo_id_, const float *lfo_amps_,
                                               int num_samples_)
{
...
void mono_AudioDeviceManager::send_adsr_message(int adsr_id_, const float *adsr_amps_,
                                                int num_samples_)
{
...
NOINLINE void mono_AudioDeviceManager::read() noexcept
{
    ...
    if (xml)
    {
        ...
        {
            open_port("RECIEVE_MIDI_MAIN",
                      xml->getStringAttribute("RECIEVE_MIDI_MAIN", DISABLED_PORT));
            open_port("RECIEVE_CC", xml->getStringAttribute("RECIEVE_CC", DISABLED_PORT));

            open_port("SEND_MIDI_CC_FEEDBACK",
                      xml->getStringAttribute("SEND_MIDI_CC_FEEDBACK", DISABLED_PORT));
            ...
            open_port("SEND_MIDI_F_ADSR_1",
                      xml->getStringAttribute("SEND_MIDI_F_ADSR_1", DISABLED_PORT));
            open_port("SEND_MIDI_F_ADSR_2",
                      xml->getStringAttribute("SEND_MIDI_F_ADSR_2", DISABLED_PORT));
            open_port("SEND_MIDI_F_ADSR_3",
                      xml->getStringAttribute("SEND_MIDI_F_ADSR_3", DISABLED_PORT));
