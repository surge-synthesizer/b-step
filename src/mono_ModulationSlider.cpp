...
void mono_ModulationSlider::refresh()
{
    ...
    {
        if ((show_value_popup || UiLookAndFeel::getInstance()->show_values_always) and
            slider_modulation->isEnabled() and !_modulator_is_own_parameter)
        {
            ...
            else
            {
                ...
                if (last_painted_mod_slider_val != modulation_value)
                {
                    ...
                    slider_modulation->SET_VALUE_TO_PAINT(String(mono_floor(modulation_value)) +
                                                          String("@") + String("%"));
                    is_repaint_required = true;
                }
                SET_SLIDER_LABEL_STYLE(slider_modulation,
    ...
    else if (show_value_popup_type == ModulationSliderConfigBase::SHOW_OWN_VALUE)
    {
        if ((show_value_popup || UiLookAndFeel::getInstance()->show_values_always))
        {
            ...
            if (slider_value->isVertical())
            {
                ...
                label_top->setText(_config->get_top_value() +
                                       String(_config->get_top_suffix().text),
            ...
            else
            {
                if (slider_value->isEnabled())
                {
                    ...
                    if (last_painted_value_slider_val != value)
                    {
                        ...
                        slider_value->SET_VALUE_TO_PAINT(_config->get_top_value() + String("@") +
                                                         String(_config->get_top_suffix().text));
                        is_repaint_required = true;
                    }
                ...
                else
                {
                    ...
                    {
                        last_painted_mod_slider_val = modulation_value;
                        slider_modulation->SET_VALUE_TO_PAINT(
                            _config->get_top_value() + String("@") +
                            String(_config->get_top_suffix().text));
                        is_repaint_required = true;
                    }
                    SET_SLIDER_LABEL_STYLE(slider_modulation,
    ...
    if (!_modulator_is_own_parameter)
        _modulator_parameter.write_modulation_to(slider_modulation,
                                                 _config->is_modulation_slider_centered());
    else
...
mono_ModulationSlider::mono_ModulationSlider(ModulationSliderConfigBase *config_)
    : _config(config_), _parameter(_config->get_parameter_compatibility_base()),
      ...
      _button_parameter(_config->get_button_parameter_compatibility_base())
{
    ...
    // INIT SLIDERS AND BUTTONS
    {
        ...
        if (!_modulator_is_own_parameter)
        {
            ...
            _modulator_parameter.init_modulation_slider(slider_modulation,
                                                        _config->is_modulation_slider_centered());
        }
        else
        {
...
void mono_ModulationSlider::sliderValueChanged(juce::Slider *sliderThatWasMoved)
{
    ...
    if (sliderThatWasMoved == slider_value)
    {
        ...
        if (_modulator_is_own_parameter)
        {
            IF_MIDI_LEARN__HANDLE_TWO_PARAMS__AND_UPDATE_COMPONENT(
                _parameter.get_param(), _modulator_parameter.get_param(), sliderThatWasMoved)
            else { _parameter.read_value_from(slider_value); }
        }
        else
        {
            IF_MIDI_LEARN__HANDLE__AND_UPDATE_COMPONENT(_parameter.get_param(), sliderThatWasMoved)
            else { _parameter.read_value_from(slider_value); }
        }
        AppInstanceStore::getInstance()->editor->show_info_popup(
            sliderThatWasMoved, _parameter.get_param()->midi_control);
    ...
    else if (sliderThatWasMoved == slider_modulation)
    {
        ...
        if (_modulator_is_own_parameter)
        {
            IF_MIDI_LEARN__HANDLE_TWO_PARAMS__AND_UPDATE_COMPONENT(
                _parameter.get_param(), _modulator_parameter.get_param(), sliderThatWasMoved)
            else { _modulator_parameter.read_value_from(slider_modulation); }
            AppInstanceStore::getInstance()->editor->show_info_popup(
                sliderThatWasMoved, _modulator_parameter.get_param()->midi_control);
        }
        else
        {
            IF_MIDI_LEARN__HANDLE__AND_UPDATE_COMPONENT(_parameter.get_param(), sliderThatWasMoved)
            else { _modulator_parameter.read_modulation_from(slider_modulation); }
            AppInstanceStore::getInstance()->editor->show_info_popup(
                sliderThatWasMoved, _parameter.get_param()->midi_control);
        }
...
void mono_ModulationSlider::buttonClicked(juce::Button *buttonThatWasClicked)
{
    ...
    else if (buttonThatWasClicked == button_modulator)
    {
        ...
        if (_button_parameter)
        {
            IF_MIDI_LEARN__HANDLE__AND_UPDATE_COMPONENT(_button_parameter.get_param(),
                                                        buttonThatWasClicked)
            else { _button_parameter.invert(); }
            AppInstanceStore::getInstance()->editor->show_info_popup(
                buttonThatWasClicked, _button_parameter.get_param()->midi_control);
        }
