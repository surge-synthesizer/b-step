...
juce_ImplementSingleton(MIDIControlHandler)
    ...
    NOINLINE MIDIControl::MIDIControl(mono_ParameterCompatibilityBase *const owner_)
    : is_in_ctrl_mode(false), owner(owner_)
{
    midi_number = -1;
    listen_type = NOT_SET;
    channel = -1;
                ...
                else
                {
                    if (is_in_ctrl_mode)
                    {
                        ...
                        else if (owner->has_modulation())
                        {
                            ...
                            // PICKUP
                            if (current_modulation + pickup > new_modulation &&
                                current_modulation - pickup < new_modulation)
                            {
...
void MIDIControl::parameter_value_changed(mono_ParameterBase<bool> *param_) noexcept
{
    ...
    bool do_send =
        ...
        ((is_ctrl_version_of && is_in_ctrl_mode) || DATA(synth_data).ctrl.get_base() == param_);
    if (do_send)
        send_standard_feedback();
}
...
// ==============================================================================
MIDIControlHandler::MIDIControlHandler() { clear(); }
...
bool MIDIControlHandler::is_waiting_for_param() const { return is_activated_and_waiting_for_param; }
...
ChangeParamOverTime::ChangeParamOverTime(mono_ParameterBase<float> &param_, float target_value_,
                                         int time_in_ms_)
    : _param(param_), _sum_callbacks(time_in_ms_ / PARAM_CHANGE_INTERVAL_IN_MS),
      _target_value(target_value_), _value_delta((target_value_ - param_) / _sum_callbacks)
{
